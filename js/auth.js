// 인증 관리 모듈
import { auth, db, functions, FCM_PUBLIC_VAPID_KEY, APP_ORIGIN, IS_LOCAL_ENV, noteFirestoreConnectivityFailure, forceFirestoreReconnect } from './firebase-config.js?v=447';
...
import { showToast, onRefreshFailure, withAsyncTimeout } from './ui-helpers.js?v=447';
import { getDatesInfo } from './ui-helpers.js?v=447';
import { escapeHtml } from './security.js?v=447';
import { applyDomTranslations, buildLocalizedUrl, getLocale, isEnglishLocale, t } from './i18n.js?v=447';
import {
...
} from './auth-login-helpers.js?v=447';
import { getAllowedTabsForMode, getDefaultTabForMode, getAppModeFromPath, getRouteContext, normalizeTabForRoute } from './app-mode.js?v=447';
import { trackProductEvent } from './product-events.js?v=447';
// blockchain-manager는 동적 import한다. 로드 실패가 인증 흐름에 영향을 주지 않게 분리한다.

const BLOCKCHAIN_MANAGER_MODULE_PATH = './blockchain-manager.js?v=447';
    ...
    onAuthStateChanged(auth, (user) => {
        if (user) {
            ...
            const initialDailyLoadPromise = window.loadDataForSelectedDate
                ? Promise.resolve(window.loadDataForSelectedDate(todayStr)).catch(onRefreshFailure('선택한 날짜 기록'))
                : Promise.resolve();
            ...
            if (window.refreshPwaActionableBadgeFromServer) {
                setTimeout(() => {
                    window.refreshPwaActionableBadgeFromServer(user).catch(onRefreshFailure('앱 배지'));
                }, 180);
            }
            ...
            getDoc(userRef).then(async userDoc => {
                ...
                if (Notification.permission === 'granted') {
                    setTimeout(() => {
                        syncCurrentPushState(user).catch(onRefreshFailure('푸시 상태'));
                    }, 400);
                }
                ...
                if (ud.adminFeedback && ud.feedbackDate) {
                    ...
                    // 닫기 표식에 메시지 날짜를 넣는다. 예전에는 uid 만 썼던 탓에
                    // 한 번 X 를 누른 회원은 그 뒤 어떤 코치 메시지도 못 봤다 —
                    // 보낸 쪽은 '3일간 노출됩니다' 를 보고 전달된 줄 알았다.
                    const isCollapsed = localStorage.getItem(`hide_fb_${user.uid}_${ud.feedbackDate}`);
                    // 7일. 보관함이 생겨 놓쳐도 다시 볼 수 있으니 짧게 둘 이유가 없다.
                    if (diffDays <= 7) {
                        window.showCoachMessage?.({
                            summary: ud.adminFeedbackSummary || '',
                            body: ud.adminFeedback,
                            feedbackDate: ud.feedbackDate,
...
// 사람은 회원 문서가 이미 만들어져 있어서 다음 로그인에 isNewUser 가 거짓이 되고,
// 아무것도 동의한 적 없는데 "약관이 바뀌었어요" 를 보게 된다.
function hasNoConsentRecord(userData = {}) {
...
const APP_VERSION_LABELS_EN = { ko: 'Full', simple: 'Simple', en: 'English', app: 'Lite' };
...
window.updateLoginButtonForVersion = function (version) {
    ...
    if (!btn) return;
    // 영문 화면에 한글 안내가 남으면 화면이 반쯤 번역된 것처럼 보인다.
    // 버튼 이름(Full/Simple/English/Lite)도 그 화면 표기를 따라야 한다.
