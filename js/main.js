...
import { initAuth, setupAuthListener } from './auth.js?v=345';
import { APP_ENV } from './firebase-config.js?v=345';
import { getActiveBscNetwork, getActiveHbtTokenAddress } from './blockchain-config.js?v=345';
...
// 무료 3일 미니 챌린지는 온체인을 타지 않으므로 블록체인 모듈이 없어도 시작된다.
...
window.startChallenge30D = (challengeId) => import('./challenge-claim.js?v=345')
    .then(mod => {
    ...
    .catch(error => {
        console.error('챌린지 시작 모듈 로드 실패:', error);
        return false;
    });
...
window.claimChallengeReward = (tier) => import('./challenge-claim.js?v=345')
