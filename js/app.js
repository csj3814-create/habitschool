...
function openTab(tabName, pushState = true) {
    ...
    if (tabName === 'gallery') {
        ...
        // 입력 폼 탭(diet, exercise, sleep)으로 전환 시 데이터 다시 로드
        // clearInputs()는 loadDataForSelectedDate 내부에서 호출됨 (중복 호출 방지)
    ...
    content.innerHTML = `
        <div class="group-stats-grid">
            <div class="group-stat-item">
                <span class="group-stat-num">${totalUsers}명</span>
...
function shareApp(platform) {
    ...
    switch (platform) {
        case 'kakao':
            window.open(`https://story.kakao.com/share?url=${encoded(url)}`, '_blank', 'noopener');
            break;
        case 'twitter':
