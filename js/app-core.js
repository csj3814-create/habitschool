...
// 프로젝트 모듈 임포트
import { app, auth, db, storage, functions, APP_ENV, APP_ORIGIN, APP_OG_IMAGE_URL, MILESTONES, MISSIONS, MISSION_BADGES, MAX_IMG_SIZE, MAX_VID_SIZE, getWeekId, noteFirestoreConnectivityFailure, isFirestoreConnectivityIssue } from './firebase-config.js?v=240';
import { applyAppModeChrome, buildAppModeUrl, buildLocalizedUrl, getAllowedTabsForMode, getAppModeFromPath, getDefaultTabForMode, getRouteContext, isSimpleMode, normalizeTabForRoute } from './app-mode.js?v=240';
import {
    isSamsungInternetUserAgent,
    parsePendingSignupOnboardingState,
    shouldAutoGrantWelcomeBonus,
    shouldShowSignupOnboarding
} from './auth-login-helpers.js?v=240';
import { formatChallengeQualificationLabel, getActiveChainKey, getActiveOnchainLabel, getChallengeCompletedDays, getChallengeDateRange, normalizeChallengeQualificationPolicy, reconcileActiveChallengesWithDailyLogs } from './blockchain-config.js?v=240';
import {
...
} from './health-connect-utils.js?v=240';
import {
...
} from './habit-groups.js?v=240';
import { reconcileMilestoneState } from './milestone-helpers.js?v=240';
import { getDatesInfo, showToast, getKstDateString } from './ui-helpers.js?v=240';
import { applyDomTranslations, getLocale, installLocaleDomObserver, isEnglishLocale, t, translateText } from './i18n.js?v=240';
import { sanitize, compressImage } from './data-manager.js?v=240';
import { createSequentialTaskQueue, getResumableUploadTimeouts } from './upload-performance.js?v=240';
import { escapeHtml, isValidStorageUrl, isPersistedStorageUrl, sanitizeText, isValidFileType, checkRateLimit } from './security.js?v=240';
import { toDateSafe, getFriendshipOtherUid, isFriendshipExpired, getEffectiveFriendshipStatus, getFriendshipName } from './friendship-utils.js?v=240';
import { requestDietAnalysis, renderDietAnalysisResult, renderDietDaySummary, renderExerciseAnalysisResult, requestSleepMindAnalysis, renderSleepMindAnalysisResult, requestBloodTestAnalysis, renderBloodTestResult, requestStepScreenshotAnalysis, requestSharedTargetClassification } from './diet-analysis.js?v=240';
import {
    ...
    normalizeDemoTab
} from './guest-demo.js?v=240';
import { trackProductEvent } from './product-events.js?v=240';
import { addCalendarDays, calculateActivityStreak, countActiveDays } from './activity-days.js?v=240';
import {
...
} from './meditation-guide.js?v=240';
import { calculateMetabolicScore, renderMetabolicScoreCard } from './metabolic-score.js?v=240';
import { loadRewardMarketSnapshot } from './reward-market.js?v=240';
import {
    SOCIAL_CHALLENGE_ACTIVITY_LOOKBACK_DAYS,
    buildSocialChallengeLookbackDateStrings,
    summarizeSocialChallengeReadinessLogs
} from './social-challenge-readiness.js?v=240';
import {
    getPreviousMonthIdFromKstDateString,
    shouldAttemptMonthlyMvpRewardFromKstDateString
} from './monthly-mvp-reward.js?v=240';
...
function renderDietProgramDashboardSummary() {
    ...
    if (!shell || !chipEl || !reminderEl || !copyEl) return;
    ...
    if (supportEl) {
        ...
        supportEl.hidden = !summary.supportTip;
    }
...
function renderDietProgramConsentModal() {
    ...
    if (!titleEl || !copyEl || !noteEl || !primaryBtn || !secondaryBtn) return;
    ...
    if (pushState.action === 'guide') {
        ...
        noteEl.textContent = '네를 누르면 홈 화면 추가와 알림 연결을 바로 진행해요.';
    ...
    primaryBtn.dataset.disabled = 'false';
    primaryBtn.disabled = _dietProgramSelectionBusy;
}
...
function setDietProgramConsentBusy(isBusy = false) {
    ...
    if (primaryBtn) {
        ...
        primaryBtn.textContent = isBusy ? '저장 중..' : '네';
    }
    if (secondaryBtn) {
        secondaryBtn.disabled = isBusy;
        if (!isBusy) secondaryBtn.textContent = '아니오';
    }
}
...
async function persistDietProgramSelection(methodId, remindersEnabled) {
    ...
    try {
        await updateDoc(userRef, {
            'programPreferences.diet': nextDietPreferences
        });
    } catch (error) {
...
window.confirmDietProgramSelectionWithNotifications = async function () {
    ...
    try {
        ...
        await persistDietProgramSelection(pending.methodId, true);
        saved = true;
        ...
        if (saved) {
            console.warn('기기 알림 연결 실패:', error?.code || '', error?.message || error);
            showToast('방법 알림은 켰어요.');
        } else {
...
function _applyDietProgramUserDataInternal(userData = null) {
    _dietProgramUserData = userData && typeof userData === 'object'
        ? { ...userData }
        : {};
    window._dietProgramUserDataSnapshot = { ..._dietProgramUserData };
    renderDietProgramProfileCard();
...
window.addEventListener('install-cta-state-changed', () => {
    renderDietProgramProfileCard();
    renderDietProgramDashboardSummary();
});
...
const BLOCKCHAIN_MANAGER_MODULE_PATH = './blockchain-manager.js?v=240';
...
function addExerciseBlock(type, data = null) {
    ...
    if (!isCardio && data && data.uploadPending) {
        div.setAttribute('data-upload-pending', 'true');
    }
    if (!isCardio && data && data.localThumbSeed) {
        div.setAttribute('data-local-thumb', data.localThumbSeed);
    }
    ...
    } else if (!isCardio && data && (data.uploadPending || data.localThumbSeed)) {
        const promptSpan = div.querySelector('.upload-area > span');
        if (promptSpan) promptSpan.style.display = 'none';
        const localThumbSeed = resolveUsableStrengthLocalThumb(data.localThumbSeed);
        if (localThumbSeed) {
            showStrengthPreviewImage(div, localThumbSeed, { localThumb: localThumbSeed });
        } else {
...
window.removeExerciseBlock = function(block) {
    ...
    const type = getExerciseBlockType(block);
    const mediaId = String(block.dataset.mediaId || '').trim();
    const hasPersistedMedia = !!String(block.getAttribute('data-url') || '').trim();
    const hasPendingMedia = !!getSelectedMediaFile(input);
...
let _removedExerciseMediaIds = {
    cardio: new Set(),
    strength: new Set()
};

function resetRemovedExerciseMediaIds() {
    _removedExerciseMediaIds = {
        cardio: new Set(),
        strength: new Set()
    };
}

function hashMediaKey(value = '') {
...
function createClientMediaId(prefix = 'media') {
    return `${prefix}_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
}

function getExerciseItemMediaId(type, item = null, index = 0) {
    const explicitId = String(item?.mediaId || '').trim();
    if (explicitId) return explicitId;
    const mediaUrl = String(item?.imageUrl || item?.videoUrl || '').trim();
    if (mediaUrl) return `legacy_${type}_${hashMediaKey(mediaUrl)}`;
    return `${type}_${index}_${createClientMediaId(type)}`;
}

function getExerciseBlockType(block) {
    return block?.classList?.contains('strength-block') ? 'strength' : 'cardio';
}

function ensureExerciseBlockMediaId(block, type = 'cardio', data = null) {
    if (!block) return '';
    const existing = String(block.dataset.mediaId || '').trim();
    if (existing) return existing;
    const mediaId = getExerciseItemMediaId(type, data);
    block.dataset.mediaId = mediaId;
    return mediaId;
}
...
function isExerciseBlockEmpty(block) {
    ...
    const hasFile = !!getSelectedMediaFile(input);
...
window.removeStaticImage = function (e, inputId, previewId, btnId, txtId) {
    ...
    if (inputEl) {
        inputEl.value = "";
        clearSelectedMediaFile(inputEl);
    }
    if (previewEl) {
        previewEl.src = "";
    ...
    if (previewEl) {
        ...
        if (exerciseBlock) {
            exerciseBlock.setAttribute('data-user-removed', 'true');
            const mediaId = String(exerciseBlock.dataset.mediaId || '').trim();
            if (mediaId) {
                _removedExerciseMediaIds[getExerciseBlockType(exerciseBlock)].add(mediaId);
            }
            exerciseBlock.removeAttribute('data-ai-analysis');
            exerciseBlock.removeAttribute('data-url');
            exerciseBlock.removeAttribute('data-thumb-url');
...
function getSelectedMediaFile(input) {
    ...
    return input ? (_selectedMediaFileFallbacks.get(input) || null) : null;
}

function clearSelectedMediaFile(input) {
    if (!input) return;
    _selectedMediaFileFallbacks.delete(input);
}
...
function clearInputs({ preserveMedia = false } = {}) {
    ...
    if (!preserveMedia) {
        document.querySelectorAll('#diet input[type="file"], #exercise input[type="file"], #sleep input[type="file"]').forEach(input => {
            input.value = '';
            clearSelectedMediaFile(input);
        });
    }
...
function applyCachedAssetHistory(uid) {
    ...
    if ((Date.now() - Number(cached.ts || 0)) >= ASSET_LS_TTL) return false;

    const hbtItems = Array.isArray(cached.hbtHistoryItems) ? cached.hbtHistoryItems : [];
    const pointItems = Array.isArray(cached.pointHistoryItems) ? cached.pointHistoryItems : [];
    if (hbtItems.length === 0 && pointItems.length === 0) return false;
...
function writeAssetHistoryCache(uid, { hbtItems = [], pointItems = [] } = {}) {
    if (!uid) return;
...
function renderAssetChallengePendingState(reason = 'user-doc-timeout') {
    ...
    if (challengeInfo) {
        challengeInfo.innerHTML = `
            <div class="challenge-ring-card tier-mini-bg challenge-ring-pending">
...
function renderAssetChallengePanel(activeChallenges = {}, todayStr = getKstDateString()) {
    ...
    if (activeTiers.length > 0) {
        ...
        for (const tier of activeTiers) {
            ...
            const stakeText = ch.stakePrincipalReturnedEarly
                ? `💰 원금 ${escapeHtml(String(ch.hbtStaked))} HBT 반환 완료`
                : ch.hbtStaked > 0
                    ? `💰 ${escapeHtml(String(ch.hbtStaked))} HBT`
                    : '🎯 무료';
            ...
            if (isClaimable) {
                ...
                const rewardText = ch.hbtStaked > 0
                    ? `${pts}P + ${formatAssetChallengeHbt(
                        (ch.stakePrincipalReturnedEarly ? 0 : Number(ch.hbtStaked || 0)) +
                        ((Number(ch.stakeBonusBasis ?? ch.hbtStaked ?? 0) * getAssetChallengeBonusBps(ch, tier)) / 10000)
                    )} HBT`
                    : `${pts}P`;
...
window.updateAssetDisplay = async function (forceRefresh = false) {
    ...
    try {
        ...
        const userDocPromise = (forceRefresh
            ? getDocFromServer(userRef).catch((serverError) => {
                const connectivityIssue = noteFirestoreConnectivityFailure(serverError, 'asset-display user-doc-server')
                    || isFirestoreConnectivityIssue(serverError);
                if (connectivityIssue) {
                    return getDoc(userRef).catch(() => _deferredAssetSnap);
                }
                throw serverError;
            })
            : getDoc(userRef)
        ...
        if (userSnap.exists()) {
                            ...
                            <div class="challenge-ring-info">
                                ...
                                <div class="challenge-ring-remain">남은 ${remain}일 · 완료 시 ${(() => {
                                    ...
                                    if (ch.hbtStaked > 0) {
                                        ...
                                        const totalHbt =
                                            (ch.stakePrincipalReturnedEarly ? 0 : Number(ch.hbtStaked || 0)) +
                                            ((Number(ch.stakeBonusBasis ?? ch.hbtStaked ?? 0) * bonusBps) / 10000);
            ...
            if (txContainer) {
                try {
                    const [txSnap, pointSnap, reactionAwardSnap, notificationSnap] = await Promise.all([
                        _p_txHistory,
                        _p_pointHistory,
...
function isMeditationSoundEnabled() {
    if (_meditationSoundEnabled !== null) {
        return _meditationSoundEnabled;
    }
    try {
        _meditationSoundEnabled = localStorage.getItem(MEDITATION_SOUND_STORAGE_KEY) === '1';
    } catch (_) {
        _meditationSoundEnabled = false;
    }
    return _meditationSoundEnabled;
}

function persistMeditationSoundEnabled(enabled) {
    _meditationSoundEnabled = !!enabled;
    try {
        localStorage.setItem(MEDITATION_SOUND_STORAGE_KEY, _meditationSoundEnabled ? '1' : '0');
    } catch (_) {}
}
...
function ensureMeditationAudioContext() {
    ...
    if (!_meditationAudioContext) {
        _meditationAudioContext = new AudioContextCtor();
    }
    if (_meditationAudioContext.state === 'suspended') {
        _meditationAudioContext.resume().catch(() => {});
    }
...
function playMeditationToneSequence(sequence = []) {
    ...
    for (const tone of sequence) {
        const durationSec = Math.max(0.05, Number(tone.durationSec || 0.12));
        const gapSec = Math.max(0, Number(tone.gapSec || 0.05));
        ...
        oscillator.stop(cursor + durationSec + 0.01);
...
function playMeditationCue(kind = '', { soft = false } = {}) {
    ...
    case 'inhale':
        playMeditationToneSequence([{
            ...
            volume: soft ? 0.13 : 1.2
        }]);
        break;
    case 'hold':
        playMeditationToneSequence([
            { frequency: 560, durationSec: soft ? 0.07 : 0.1, gapSec: 0.05, volume: soft ? 0.12 : 1.2 },
            { frequency: 560, durationSec: soft ? 0.07 : 0.1, gapSec: 0.04, volume: soft ? 0.12 : 1.2 }
        ]);
        break;
    case 'exhale':
        playMeditationToneSequence([{
...
function getMeditationRunningCueInfo(state = _meditationUiState) {
...
function setMeditationCueTokenFromState(state = _meditationUiState) {
    const cueInfo = getMeditationRunningCueInfo(state);
    _meditationLastCueToken = cueInfo?.token || '';
}

function maybePlayMeditationRunningCue({ force = false } = {}) {
    const cueInfo = getMeditationRunningCueInfo(_meditationUiState);
    if (!cueInfo) {
...
function renderMeditationUi({ skipGuideUpdate = false } = {}) {
    ...
    const phaseStepsEl = document.getElementById('meditation-phase-steps');
...
function startMeditationTimerTick() {
    ...
    _meditationTimerTickHandle = window.setInterval(() => {
        ...
        maybePlayMeditationRunningCue({ force: false });
...
function _getRecordGuideStates() {
    ...
    const { todayStr } = getDatesInfo();
    const selectedDateStr = String(document.getElementById('selected-date')?.value || todayStr).trim() || todayStr;
    const dietGuide = buildDietProgramGuideState(getCurrentDietProgramPreferences(), {
        dietPhotoCount: dietPhotos,
        fastingMetricsCount,
        dailyLog: getSelectedDietProgramDailyLog(),
        dateStr: selectedDateStr,
        todayStr,
        nowMs: Date.now()
    });
    ...
    return {
        diet: {
            ...
            status: dietGuide.status,
            helper: dietGuide.helper
        },
        exercise: {
...
window.toggleMeditationSound = function() {
    const nextEnabled = !isMeditationSoundEnabled();
    if (nextEnabled) {
        ensureMeditationAudioContext();
    }
...
function _renderDashboardWithData(data, todayStr, weekStrs, currentWeekId, user) {
    try {
        const ud = data.ud || {};
        window.applyDietProgramUserData?.(ud);
...
function persistSavedExerciseBlock(block, url, thumbUrl) {
    ...
    block.removeAttribute('data-upload-pending');
...
function getPendingUploadSnapshot(inputId) {
    if (!inputId) return null;
    const entry = _pendingUploads.get(inputId);
    if (!entry) return null;
    return {
...
function ensureBackgroundUploadStatusEl() {
    ...
    el.id = 'background-upload-status';
...
function setBackgroundUploadStatus({ visible = false, complete = false, error = false, title = '', progress = 0 } = {}) {
    ...
    if (_backgroundUploadStatusTimer) {
        clearTimeout(_backgroundUploadStatusTimer);
...
function hasMediaUrl(value) {
    return isPersistedStorageUrl(value);
}

function calculateAwardedPointsFromLogData(logData = {}, previousAwarded = null) {
    const awardedSeed = previousAwarded || logData.awardedPoints || {};
    const awarded = { ...awardedSeed };
    ...
    const strengthCount = (Array.isArray(exercise.strengthList) ? exercise.strengthList : [])
        .filter(item => hasMediaUrl(item?.videoUrl)).length;
    if (strengthCount >= 1) newExerPts += 10;
    if (strengthCount >= 2) newExerPts += 5;
    newExerPts = Math.min(newExerPts, 30);

    let newMindPts = 0;
    if (hasMediaUrl(sleepAndMind.sleepImageUrl)) newMindPts += 10;
    if (sleepAndMind.meditationDone || sanitizeText(sleepAndMind.gratitude || '', 500)) newMindPts += 10;
    newMindPts = Math.min(newMindPts, 20);

    const oldDietPts = Number(awardedSeed.dietPoints || 0);
    const oldExerPts = Number(awardedSeed.exercisePoints || 0);
    const oldMindPts = Number(awardedSeed.mindPoints || 0);
...
function mergeExerciseItems(type, previousItems = [], currentItems = [], removedIds = null) {
    const removed = removedIds || _removedExerciseMediaIds[type] || new Set();
    const merged = new Map();

    (Array.isArray(previousItems) ? previousItems : []).forEach((item, index) => {
        const normalized = normalizeExerciseItem(type, item, index);
        if (!normalized || removed.has(normalized.mediaId)) return;
        merged.set(normalized.mediaId, normalized);
    });

    (Array.isArray(currentItems) ? currentItems : []).forEach((item, index) => {
        const normalized = normalizeExerciseItem(type, item, index);
        if (!normalized || removed.has(normalized.mediaId)) return;
        merged.set(normalized.mediaId, normalized);
    });
...
function upsertExerciseItem(type, previousItems = [], nextItem = null) {
    const normalized = normalizeExerciseItem(type, nextItem);
    if (!normalized) return mergeExerciseItems(type, previousItems, [], new Set());
    const merged = new Map();
    (Array.isArray(previousItems) ? previousItems : []).forEach((item, index) => {
        const current = normalizeExerciseItem(type, item, index);
        if (current) merged.set(current.mediaId, current);
    });
    merged.set(normalized.mediaId, normalized);
    return Array.from(merged.values());
}
...
async function applyBackgroundMediaPatch({ userId, docId, job, result, updateGallery = true, baseData = null }) {
    ...
    if (job.kind === 'diet') {
        ...
        if (_removedExerciseMediaIds.cardio.has(job.mediaId)) return false;
        const nextItem = {
            mediaId: job.mediaId,
            imageUrl: result.url,
        ...
        nextData.exercise = {
            ...(nextData.exercise || {}),
            cardioList: upsertExerciseItem('cardio', nextData?.exercise?.cardioList || [], nextItem)
        };
    } else if (job.kind === 'strength') {
        if (_removedExerciseMediaIds.strength.has(job.mediaId)) return false;
        const nextItem = {
            mediaId: job.mediaId,
            videoUrl: result.url,
        ...
        nextData.exercise = {
            ...(nextData.exercise || {}),
            strengthList: upsertExerciseItem('strength', nextData?.exercise?.strengthList || [], nextItem)
        };
    } else {
        return false;
    }
...
function upsertOfflineOutboxPendingExerciseMedia(data = {}, mediaItem = null) {
    const kind = String(mediaItem?.kind || '').trim();
    if (kind !== 'cardio' && kind !== 'strength') return data;
    ...
    const listKey = kind === 'strength' ? 'strengthList' : 'cardioList';
    const list = Array.isArray(data.exercise[listKey])
        ? [...data.exercise[listKey]]
        : [];
    const existingIndex = list.findIndex((item) => String(item?.mediaId || '').trim() === mediaId);
    const existing = existingIndex >= 0 && list[existingIndex] && typeof list[existingIndex] === 'object'
        ? { ...list[existingIndex] }
        : null;

    const persistedUrl = kind === 'strength' ? existing?.videoUrl : existing?.imageUrl;
    if (hasMediaUrl(persistedUrl)) {
        data.exercise[listKey] = list;
        return data;
    }
    ...
    if (kind === 'strength') {
        const localThumbSeed = resolveUsableStrengthLocalThumb(mediaItem?.localThumbSeed);
        if (localThumbSeed) pendingItem.localThumbSeed = localThumbSeed;
    }

    if (existingIndex >= 0) list[existingIndex] = pendingItem;
    else list.push(pendingItem);
    data.exercise[listKey] = list;
    return data;
}

function mergeOfflineOutboxPendingMediaItems(data = {}, mediaItems = []) {
    if (!Array.isArray(mediaItems) || mediaItems.length === 0) return data;
    const merged = cloneDailyLogData(data);
    mediaItems.forEach((mediaItem) => {
        upsertOfflineOutboxPendingExerciseMedia(merged, mediaItem);
    });
    return merged;
}
...
function mergeOfflineOutboxData(baseData = {}, entry = null) {
    ...
    return mergeOfflineOutboxPendingMediaItems(mergedData, entry.mediaItems);
}
...
document.getElementById('saveDataBtn').addEventListener('click', () => {
    ...
    (async () => {
        ...
        try {
            ...
            const ensureDeferredImageUpload = (inputId, file, folder) => {
            ...
            const getImmediateMediaResult = async ({ inputId, previewEl, oldUrl, oldThumbUrl, folder, backgroundKind, slot = '' }) => {
                ...
                const selectedFile = getSelectedMediaFile(inputEl);
            ...
            const cardioCurrentItems = [];
            cardioBlocks.forEach((block, index) => {
                const fileInput = block.querySelector('.exer-file');
                const mediaId = ensureExerciseBlockMediaId(block, 'cardio');
                ...
                } else if (getSelectedMediaFile(fileInput)) {
                    const selectedFile = getSelectedMediaFile(fileInput);
                    if (!pendingSnapshot) {
                ...
                if (item) {
                    ...
                    cardioPersistResults.set(block.id, item);
                }
            });
            ...
            const bThumbUrl = breakfastResult.thumbUrl, lThumbUrl = lunchResult.thumbUrl, dThumbUrl = dinnerResult.thumbUrl, sThumbUrl = snackResult.thumbUrl;
            ...
            if (backgroundJobs.length > 0) {
                runBackgroundMediaSyncJobs({
                    ...
                    onSettled: ({ failed, latestCommittedData } = {}) => {
...
function buildHabitGroupDashboardRow(group, { joined = false, checkedIn = false, checkin = null, progress = null, canJoin = true, showUnavailableAction = true } = {}) {
    ...
    const actionHtml = reviewStatus === 'approved'
        ...
        ? '<button type="button" class="social-challenge-cta is-muted" disabled>최대 2모임</button>'
        : '';
...
function buildHabitGroupRecommendationSection(rows = [], { canJoinMore = true } = {}) {
    if (!Array.isArray(rows) || rows.length === 0) return '';

    if (!canJoinMore) {
        const expanded = !!_habitGroupRecommendationsExpandedWhenFull;
        return `
            <button type="button" class="community-more-btn" style="margin-top:10px;" onclick="toggleHabitGroupRecommendationsWhenFull()">
                ${expanded ? '다른 소모임 접기' : '다른 소모임 보기 · 최대 2모임'}
            </button>
            ${expanded ? `
...
window.toggleHabitGroupRecommendationsWhenFull = function() {
    _habitGroupRecommendationsExpandedWhenFull = !_habitGroupRecommendationsExpandedWhenFull;
    _renderSocialChallengesPromise = null;
    _renderSocialChallengesPromiseUid = '';
    renderSocialChallenges(auth.currentUser).catch(error => {
        logOptionalDataTimeout('habit_group_recommendations_toggle_failed', error);
    });
};
    ...
    try {
        ...
        const recommendedRows = getRecommendedHabitGroups(getCachedDailyLog(`${user.uid}_${todayStr}`) || {}, joinedGroupIds)
            ...
            .map(group => buildHabitGroupDashboardRow(group, {
                ...
                checkedIn: false,
                canJoin: canJoinMore,
                showUnavailableAction: canJoinMore
            }));
