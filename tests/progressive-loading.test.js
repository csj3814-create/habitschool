...
describe('progressive loading isolation', () => {
    ...
    it('bounds optional asset and reward-market enrichment work', () => {
        ...
        expect(appSource).toContain('return chainKey === ACTIVE_WALLET_CHAIN_KEY;');
        expect(appSource).toContain('function appendOnchainHbtHistoryItems');
        expect(appSource).toContain('hbt Firestore history deferred; using onchain/cache fallback');
        ...
        expect(appSource).toContain('현재 비율은 100P =');
        expect(appSource).not.toContain('현재 ${chainLabel} 온체인 비율은');
        ...
        expect(appSource).toContain('_assetHistoryState.hbtDeferred = _assetHistoryState.hbtItems.length === 0 && (userDocDeferred || retryScheduled);');
        expect(appSource).toContain('거래 기록을 다시 확인하고 있어요');
        expect(appSource).toContain('window.updateAssetDisplay?.(true);');
        ...
        expect(appSource).not.toContain('clearAssetRetry(uid);\n        writeAssetDisplayCache(uid, {');
        expect(appSource).not.toContain('clearAssetRetry(user.uid);\r\n                        writeAssetDisplayCache(user.uid, {');
        expect(appSource).not.toContain('clearAssetRetry(user.uid);\n                        writeAssetDisplayCache(user.uid, {');
        ...
        expect(appSource).toContain('const shouldWaitForServerChallengeLogs = challengeProgressDates.length > 0');
        expect(appSource).toContain('if (log) updateDailyLogCache(`${uid}_${date}`, log);');
        ...
        expect(appSource).toContain('const hbtHistoryDeferred = !txSnap;');
        expect(appSource).toContain('const pointHistoryDeferred = !pointSnap || !reactionAwardSnap || !notificationSnap;');
        expect(appSource).toContain('const hasHistorySnapshotGap = hbtHistoryDeferred || pointHistoryDeferred;');
