...
- **키스토어 경로가 문서와 달랐다.** `해빛스쿨 - Google Play package` 폴더가
  ...
  (gitignored).
- ~~아직 트랙에 업로드하지 않았다.~~ **(2026-09-10 정정)** 실제로는 8/31 당일
  업로드했다. Play Console 비공개 테스트 트랙이 `버전: 6 (1.0.3) · 최종 업데이트:
  2026년 8월 31일` 로 확인된다. 이 문장을 그대로 믿고 "versionCode 6 을 올려야
  한다"고 판단했다가 되돌렸다 — **콘솔에서 확인한 사실이 문서보다 우선한다.**
- 그리고 그 뒤로 **트랙이 10일째 멈춰 있다.** 반려 사유 2번은 이 창에서 업데이트가
  쌓여야 풀리는데, 8/31 의 versionCode 6 은 `build.gradle.kts` 의 버전 번호 두 줄만
  바꾼 빈 빌드였다(`19665d7`). 9/12 재신청 전에 내용 있는 versionCode 7 이 필요하다.
