...
- A twelfth follow-up fix made the UX degrade more honestly while old thumb-less records still exist: if the live video fallback is active, `썸네일 제작중` is cleared once the frame is visible, and share-card media now skips unresolved exercise videos instead of rendering a blue placeholder tile.
