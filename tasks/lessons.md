...
## 2026-04-05 (대시보드 단순화 체감)

### 61. 대시보드 단순화는 상단 CTA만 줄이지 말고 하위 모듈 카드와 보조 도구까지 같이 정리해야 사용자가 변화를 느낀다
- **증상**: 내 기록 탭을 단순화했다고 반영했지만 사용자는 여전히 버튼이 겹치고 길이가 길어서 "변한 게 없다"고 느꼈다.
- **근본 원인**: 상단 요약 카피와 일부 버튼만 줄였고, 실제 길이를 만드는 `친구 체크 / 친구 챌린지 / 커뮤니티 / 마일스톤 / 결과지` 하위 블록은 그대로 남아 있었다.
- **교훈**: 모바일 컴팩트 개편에서는 카피보다 먼저 DOM 레벨에서 중복 모듈을 제거해야 한다. 한 박스에는 한 행동만 남기고, 보조 정보는 접기나 별도 화면으로 빼야 체감이 생긴다.

## 2026-04-05 (내 기록 탭 중복 정리 2차)

### 60. 대시보드 단순화는 카드 수만 줄이지 말고 실제 CTA 중복과 클릭 경로까지 함께 검증해야 한다
- **증상**: `오늘의 루틴`과 `오늘 남은 행동`이 같은 역할을 하고, `함께하기`도 제목/본문/버튼이 중복되어 사용자가 무엇을 눌러야 하는지 다시 해석해야 했다. 친구 초대 버튼도 존재했지만 실제로는 QR 의존 경로라 사용자가 체감상 "안 되는 버튼"처럼 보였다.
- **교훈**: 대시보드 단순화 작업에서는 1) 같은 결정을 요구하는 카드가 두 개 남아 있지 않은지, 2) CTA가 실제로 가장 안정적인 목적지로 연결되는지, 3) 모바일 첫 두 화면 안에서 핵심 행동과 주간 미션 설정이 끝나는지를 staging에서 실제 눌러서 확인해야 한다.

## 2026-04-05 (staging 체감 검증과 캐시)

### 61. 구조 개편은 코드 diff보다 staging에서 실제 보이는 박스 기준으로 검증해야 한다
- **증상**: 로컬 코드에서는 중복 카드를 뺐는데도 staging/PWA 캐시 화면에서는 예전 박스와 겹치는 버튼이 그대로 보여 사용자가 "변한 게 없다"고 느꼈다.
- **교훈**: 대시보드 구조 변경에서는 1) service worker 캐시 버전을 함께 올리고, 2) staging에서 실제 첫 화면 기준으로 남아 있는 박스를 다시 세고, 3) 제거 대상이 렌더 함수뿐 아니라 DOM 배치에도 남아 있지 않은지 확인해야 한다.

### 62. 모바일 버튼은 폭만 100%로 키우지 말고 컨테이너 방향까지 같이 바꿔야 한다
- **증상**: 주간 미션 박스에서 `식단 기록` 버튼이 가로폭을 다 차지하면서 왼쪽 설명 영역이 세로 글자처럼 찌그러졌다.
- **교훈**: 모바일에서 CTA를 넓히려면 버튼 자체만 조정하지 말고 부모 flex 컨테이너를 세로 배치로 바꾸고, 설명 영역에 충분한 폭을 남기는지 실제 화면으로 확인해야 한다.

### 63. 선택지가 3개처럼 적고 고정일 때는 드롭다운보다 버튼형이 더 빠르다
- **증상**: 주간 미션 난이도를 드롭다운으로 바꾸자 화면은 짧아졌지만, 사용자는 `쉬움 / 보통 / 도전`을 한눈에 비교하고 바로 누르기 어려워졌다.
- **교훈**: 모바일 컴팩트화에서도 선택지가 적고 의미 비교가 중요한 경우에는 숨기기보다 버튼형 세그먼트가 낫다. 높이 축소와 선택 속도 중 충돌이 나면, 먼저 실제 선택 속도를 지키고 그다음 패딩과 줄 수를 줄인다.
## 2026-04-05 (친구 초대 코드와 카카오 연결 코드 혼선)

### 60. 친구 초대 코드와 계정 연결 코드는 UI에서 같은 종류처럼 보이면 안 된다
- **증상**: 프로필에 `연결 코드` 카드와 `내 초대 코드` 카드가 같이 보여서, 사용자가 친구 연결용 코드를 또 새로 만드는 것으로 이해했다.
- **근본 원인**: 실제로는 `referralCode`가 친구 요청용, `chatbotLinkCode`가 카카오 해빛코치 계정 연결용 1회성 코드인데, UI 문구가 이를 충분히 구분하지 못했다.
- **교훈**: 친구 초대, 계정 연결, 인증 같이 코드형 기능이 여러 개 있을 때는 목적을 이름에 직접 써야 한다. `연결 코드` 같은 포괄 표현 대신 `카카오 등록 코드`, `친구 초대 코드`처럼 용도를 바로 보여주고, 서로 다른 코드 흐름이 같은 일처럼 읽히지 않게 설계할 것.

### 61. 핵심 요약 지표는 작은 화면에서도 의미 구조를 유지해야 한다
- **증상**: `연속 기록 / 오늘 완료 / 이번 주 인증 / 다음 보상` 카드가 모바일에서 1열로 길게 쌓여 화면을 불필요하게 늘렸다.
- **근본 원인**: `max-width: 360px` 예외에서 요약 카드를 1열로 바꾸는 CSS가 남아 있었고, 이 지표 묶음이 한 세트라는 정보 구조를 깨뜨렸다.
- **교훈**: 핵심 KPI 4개처럼 묶음으로 읽혀야 하는 요약 카드는 초소형 화면에서도 가능한 한 `2x2` 구조를 유지해야 한다. 너무 좁을 때는 열 수를 줄이기보다 패딩과 글자 크기를 먼저 줄일 것.

### 62. 내 기록 핵심 박스는 접기보다 본문에서 바로 보여주는 편이 낫다
- **증상**: `이번 주 흐름`과 `나의 마일스톤`이 `기록 더보기` 안으로 들어가면서, 사용자가 내 기록 탭에서 바로 봐야 할 정보를 다시 한 번 열어야 했다.
- **근본 원인**: 화면 길이를 줄이려는 과정에서 2차 정보와 핵심 회고 정보를 같은 접힘 영역으로 묶어 버렸다.
- **교훈**: 내 기록 탭에서는 현재 진행과 회고에 직접 연결되는 박스는 기본 노출로 두고, 정말 보조적인 도구만 접을 것. 길이를 줄일 때는 먼저 중복 CTA와 문구를 줄이고, 핵심 카드 visibility는 마지막에 건드릴 것.
## 2026-04-05 (공유 카드 캡처 안정화)

### 64. html2canvas로 공유 카드를 만들 때는 캡처 전에 이미지가 실제로 decode된 상태인지 먼저 보장해야 한다
- **증상**: 공유 카드에서 식단/운동 이미지가 화면에는 보이는데, 실제 공유된 PNG에는 빈 박스처럼 빠져 보였다.
- **근본 원인**: 카드 DOM을 그린 직후 `html2canvas`를 호출하면서, 내부 `img`가 아직 완전히 준비되지 않았거나 원격 URL이 그대로 남아 캡처 시점에 누락됐다.
- **교훈**: 공유용 카드 캡처에서는 1) 현재 DOM 안의 `img`를 먼저 기다리고, 2) 가능하면 data URL로 고정한 뒤, 3) 그 다음에 `html2canvas`를 호출해야 한다. 원격 fetch가 실패해도 빈 박스가 아니라 완성형 플레이스홀더가 보이도록 마지막 폴백까지 남겨야 한다.
## 2026-04-05 (공유 카드 미리보기/캡처 안정화)

### 60. 공유 미리보기와 실제 캡처는 같은 준비된 이미지 소스를 써야 한다
- **증상**: 공유 카드 박스에서는 이미지가 깨지고, `공유하기`를 누르면 `이미지 생성 중...`에서 멈췄다.
- **근본 원인**: 페이지 미리보기는 원격 Storage URL을 그대로 그리고, 캡처 시점에는 다시 별도 fetch/base64 변환을 하면서 느린 응답 하나가 전체 흐름을 붙잡고 있었다.
- **교훈**: 공유 기능은 `미리보기용 안전 썸네일 준비 -> 그 결과로 캡처` 한 줄 흐름으로 고정해야 한다. 원격 URL을 바로 그리지 말고, 타임아웃이 있는 준비 단계에서 data URL 또는 플레이스홀더로 먼저 확정한 뒤 미리보기와 최종 이미지가 같은 소스를 쓰게 만들어야 한다.

### 61. 공유 미리보기와 캡처는 같은 DOM을 바로 재사용하지 말고 역할을 분리해야 한다
- **증상**: 깨진 이미지를 막으려고 안전한 플레이스홀더 위주로 바꾸자, 이번에는 미리보기 카드에서도 실제 사진이 사라졌다.
- **근본 원인**: 화면에 보여주는 카드와 공유 순간에만 필요한 캡처 안정화 로직을 한 경로로 묶어서, 캡처용 폴백이 미리보기 UX까지 덮어썼다.
- **교훈**: 사용자가 보는 미리보기는 실제 이미지 우선, 공유 순간에는 캡처 안정성 우선으로 설계해야 한다. 즉 `보이는 카드`와 `캡처용 준비 단계`는 분리하고, 공유 후에는 화면 카드를 원래 상태로 복원하는 흐름을 기본값으로 삼는다.

### 62. 모바일 반응 버튼은 텍스트 길이를 기준으로 강제로 한 줄 고정해야 한다
- **증상**: 갤러리 카드의 `좋아요` 버튼이 모바일에서 두 줄로 꺾여 보여서 버튼 높이와 리듬이 깨졌다.
- **근본 원인**: 아이콘, 라벨, 숫자 배지를 같은 inline 흐름에만 두고 줄바꿈 제약을 명시하지 않아 폭이 조금만 부족해도 라벨이 아래 줄로 밀렸다.
- **교훈**: 4칸 반응 버튼처럼 폭이 좁은 UI는 `아이콘 / 라벨 / 배지`를 명시적인 슬롯으로 나누고, `white-space: nowrap`, 작은 폰트, 고정 shrink 규칙을 같이 줘서 한 줄 배치를 강제해야 한다.

## 2026-04-05 (공유 카드 미리보기/실제 파일 불일치 수정)

### 공유 미리보기와 실제 공유 파일은 같은 렌더러를 써야 한다
- 증상: 화면에서는 카드가 보이는데 공유 파일은 깨지거나, 반대로 공유 박스는 placeholder인데 실제 파일 생성은 멈췄다.
- 근본 원인: DOM 미리보기와 html2canvas 캡처가 서로 다른 경로를 타면서 외부 이미지 로딩/CORS 실패 양상이 달랐다.
- 교훈: 공유 기능은 미리보기와 실제 파일 생성을 같은 캔버스 렌더러로 통일하고, preview 전용 <img>에는 불필요한 crossorigin 강제를 넣지 않는다.