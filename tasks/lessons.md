...
## 2026-09-10 (Diagnosing a Build That Was Never Installed)

### 256. 기기에서 동작을 진단하기 전에 설치된 빌드의 버전부터 확인한다
- 증상: versionCode 7 에 넣은 "앱 열 때 걸음수 자동 읽기" 가 폰에서 전혀 동작하지
  않았다. 콜드 런치도, 복귀도 안 됐다. 원인을 찾겠다고 웹의 URL 파싱, 런처의
  태스크 복귀 동작, 삼성 헬스 동기화 지연까지 네 번에 걸쳐 가설을 세우고 사용자에게
  확인을 요청했다. **전부 헛다리였다. 폰에 깔린 것은 1.0.3(versionCode 6) 이었고,
  진단하려던 코드가 애초에 기기에 없었다.**
- 원인: "앱이 열린다"를 "새 빌드가 깔렸다"로 읽었다. 내부 테스트에 올렸다는 사실만
  보고 그것이 기기에 반영됐다고 가정했다. Play 는 트랙에 올렸다고 바로 주지 않는다 —
  출시가 완료돼야 하고, 계정이 그 트랙의 테스터여야 한다.
- 교훈: 네이티브 동작을 진단할 때 **첫 질문은 "설치된 버전이 무엇인가"** 다.
  `설정 → 애플리케이션 → 앱 → 버전` 한 줄이면 끝나고, USB 도 필요 없다.
  그 확인 없이 세운 가설은 전부 무효다. 웹 배포는 즉시 반영되지만 **APK 는 그렇지
  않다** — 같은 세션에서 웹과 앱을 함께 고칠 때 이 비대칭이 사람을 속인다.
- 덧: 이번 실패의 비용은 사용자의 시간이었다. 헬스커넥트 권한 화면, 위젯, 완전 종료,
  10분 대기까지 시켜 놓고 답은 버전 한 줄에 있었다. **사용자에게 확인을 요청하기
  전에, 내가 확인할 수 있는 것과 한 줄로 끝나는 것을 먼저 배치한다.**

## 2026-09-10 (Dormant Flag vs. Store Review Calendar)

### 255. 꺼져 있는 기능을 되살릴 때는 코드보다 스토어 달력을 먼저 읽는다
- 증상: "기기 연동을 넓히자"는 요청에 Health Connect 권한 10개를 추가하는 계획을
  냈다. 사용자가 "이거 진행하면 플레이스토어 등록에 문제 생긴다고 했던 것 같은데"
  라고 되물어 다시 확인하니, **8/29 프로덕션 액세스 반려 · 9/12 재신청 대기** 상태였다.
  이틀 뒤 재신청을 앞두고 데이터 보안 선언과 건강 권한 선언을 새로 쓰게 만드는
  계획이었다.
- 원인: 저장소 코드만 읽고 `tasks/` 의 운영 기록을 읽지 않았다. 코드는 "이 기능을
  켤 수 있다"까지만 말하고, **켜도 되는 때인지는 말하지 않는다.** 그 답은
  `tasks/2026-08-*_play_*.md` 에 전부 적혀 있었다.
- 교훈: 배포 대상이 앱 스토어를 거치는 변경은 **`tasks/` 의 최신 운영 문서부터
  읽는다** — 심사 상태, 반려 이력, 재신청 날짜. 그리고 계획을 스토어 마일스톤
  기준으로 자른다. 이번에는 그렇게 자르고 나니 **걸음 수 되살리기는 새 권한이
  0개**라는 것이 드러났다(`READ_STEPS` 는 f9d0d81 이후 모든 AAB 에 있었다).
  위험한 절반과 안전한 절반을 나누면, 기다리지 않아도 되는 쪽이 남는다.
- 덧: 서브에이전트가 보고한 릴리스 버전(`v=377`)이 실제(`v=380`)와 달랐다.
  **에이전트 보고의 숫자는 쓰기 직전에 원본에서 다시 확인한다.**

## 2026-08-31 (LE8 Blank Item Leads Nowhere)

### 254. 빈 칸으로 데려가는 안내는 그 칸이 화면에 있을 때만 안내다
- 증상: 건강습관 점수의 "흡연 상태 입력 필요 / 체중(BMI) / 콜레스테롤 수치 필요" 줄을 눌러도 아무 일도 일어나지 않는다는 제보(v348, prod).
- 원인: 그 칸들은 건강정보 동의 전에는 `#sensitive-consent-cards` 채로 `hidden` 이다. `getElementById` 는 감춰진 요소도 찾아내므로 코드는 성공한 것처럼 보이지만, `scrollIntoView()` 도 `focus()` 도 감춰진 요소에는 아무 일을 하지 않는다. 화면이 그대로라 사용자에게는 "눌러도 입력이 안 되는 앱"이 된다.
- 교훈: 특정 입력칸으로 데려가는 동선은 대상이 **화면에 있는지**(`offsetParent`/`checkVisibility`)까지 확인하고, 없으면 감춘 쪽이 왜 막혔는지 말하게 넘긴다. 그리고 무언가를 잠갔으면 **잠금 문구에 그 안에 무엇이 있는지 전부 적는다** — "체성분·약물·혈액검사"만 적힌 자물쇠 뒤에 흡연 칸이 들어가면 아무도 그 자물쇠를 열어 보지 않는다. 새 카드를 동의 묶음 안에 넣을 때는 그 카드를 가리키는 다른 동선이 있는지 함께 본다.

## 2026-07-19 (Samsung Video Thumbnail Validation)

### 253. 캔버스 출력 성공과 실제 썸네일 성공은 다르다
- 증상: 운동 영상을 저장해도 삼성 휴대폰에서 검은 화면이 썸네일로 남았고, 미리보기 유지 수정만으로는 해결되지 않았다.
- 원인: 첫 디코딩 프레임의 `data:image/...` 생성 여부만 검사해 거의 균일한 검은 프레임도 정상 썸네일로 채택하고 업로드했다.
- 교훈: 모바일 영상 썸네일은 여러 재생 시점에서 디코딩을 기다린 뒤 픽셀 밝기·분산으로 빈 검은 프레임을 거르고, 실제 화면 정보가 있는 프레임만 저장해야 한다. 소스 문자열 테스트만으로 완료 판단하지 말고 검은 프레임 거부와 후속 프레임 선택을 테스트한다.

## 2026-07-18 (HabitSchool Brand Role)

### 252. 해빛스쿨은 최석재 브랜드의 부가 경력이 아니라 행동 변화의 핵심 제품이다
- 증상: 브랜드 계획에서 해빛스쿨을 생활습관 앱 운영 경험을 보여주는 보조 증거로만 배치했다.
- 원인: 대외 인지도와 강의 전환에 집중하면서, 건강 정보를 접한 사람이 목표를 세우고 반복 행동을 통해 삶을 바꾸는 제품의 본래 역할을 축소했다.
- 교훈: 최석재의 콘텐츠와 강의는 인식·판단을 만들고, 해빛스쿨은 목표 설정·기록·지속·공동체·변화 확인을 담당하는 핵심 실행축으로 설계한다. 제품과 브랜드의 성과는 노출량뿐 아니라 첫 기록 전환, 4주·8주 지속, 참여자가 직접 기록한 변화로 측정한다.
...
- 2026-04-21: 초 단위로 진행되는 애니메이션 UI는 `setInterval` 렌더마다 DOM을 통째로 다시 그리면 안 된다. 호흡 단계 같은 진행 UI는 단계 구조가 바뀔 때만 마크업을 만들고, 매초 렌더에서는 활성 상태만 갱신해야 `4초 들이쉼/6초 내쉼`처럼 긴 애니메이션이 매초 리셋되지 않는다.
- 2026-04-21: DOM 재사용만으로는 실제 시간 기반 호흡 애니메이션이 충분하지 않을 수 있다. `들이쉼/내쉼`처럼 단계 길이가 중요한 UI는 키프레임 모양으로 흉내내지 말고, 현재 단계 진행률을 계산해 `fill` 값을 직접 갱신하고 그 사이만 CSS transition으로 보간해야 사용자 체감이 단계 초와 맞는다.
- 2026-04-21: 같은 `멈춤`이라도 의미가 다르면 시각 상태를 나눠야 한다. 4-7-8의 멈춤, 박스호흡의 꽉 찬 멈춤/다 빠진 멈춤처럼 UX 차이가 있는 경우에는 라벨 하나로 뭉개지 말고 `hold-full`, `hold-empty` 같은 별도 메타 값으로 분기해 렌더링과 사운드가 정확히 따라가게 한다.
- 2026-04-21: 명상 보조 영상처럼 사용자 선택이 있는 임베드는 정적 `iframe` 하나로 끝내지 않는다. 선택칩, 현재 선택 링크, autoplay URL, fullscreen 종료 복구까지 한 세트로 묶고, 마음챙김이 아닐 때는 autoplay를 꺼서 다른 명상 흐름에 영상 상태가 새지 않게 해야 한다.
- 2026-04-21: 사용자에게 여러 영상 선택지를 준다면 재생목록 하나의 `index`만 바꾸는 방식으로 때우지 않는다. 브라우저/임베드 캐시에 따라 같은 영상처럼 보일 수 있으니, 선택지마다 서로 다른 고정 `videoId`를 두고 기본 iframe src와 열기 링크도 같은 소스를 가리키게 맞춘다.
- 2026-04-21: 영상 선택칩은 기술적으로만 서로 다른 소스를 연결해 두지 말고, 실제 영상 분위기와 사용자 노출 이름이 맞는지 마지막에 한 번 더 대조한다. 특히 명상/휴식 콘텐츠는 라벨이 분위기를 설명하는 역할을 하므로 소스 교체 뒤에는 칩 이름도 같이 재점검해야 한다.
- 2026-04-21: 카드 안 보조 미디어는 이미 본문 카드가 충분히 크면 별도 헤더 박스를 또 씌우지 않는다. 마음챙김처럼 카드 하나 안에서 끝나는 경험은 선택칩과 플레이어만 남기고, 중복 제목/외부 링크는 기본에서 빼서 흐름을 단순하게 유지한다.
- 2026-04-21: `hidden` 속성으로 토글하는 UI에 클래스 `display: grid/flex/block`를 주면 author CSS가 `hidden`을 덮을 수 있다. 마음챙김 영상처럼 조건부 섹션은 `[hidden] { display:none !important; }`를 같이 두거나 inline style로 제어해서 "조건은 맞는데 계속 보이는" 회귀를 막는다.
- 2026-04-21: CSS transition이 걸린 fill UI는 새 단계 시작값을 그대로 `setProperty`하면 이전 상태에서 새 시작값까지도 애니메이션돼 보일 수 있다. 호흡 단계처럼 `들이쉼/내쉼` 시작 모양이 중요한 UI는 단계 시작 시 transition을 잠깐 `0s`로 끄고 시작 fill을 먼저 적용한 뒤, 다음 프레임에만 실제 목표 fill로 보간해야 역방향 튐을 막을 수 있다.
- 2026-04-21: YouTube 임베드는 iframe만 허용해도 콘솔이 조용해지지 않을 수 있다. 플레이어/미리보기 이미지가 `i.ytimg.com`에서 내려오므로, 영상 UI를 넣을 때는 `frame-src`뿐 아니라 `img-src`에 `https://*.ytimg.com`까지 함께 넣어 CSP 잡음을 막는다.
- 2026-04-23: 한국 규제 리스크가 있는 보상 설계는 온체인 토큰과 기프티콘 교환을 초기에 바로 결합하지 않는다. 사용자가 포인트 기반 교환으로 방향을 교정하면 보상몰 결제 자산을 오프체인 포인트로 먼저 분리하고, 온체인 HBT는 별도 자산 흐름으로 유지하는 구조를 우선 적용한다.
- 2026-04-26: 사용자가 보상 교환 비율을 다시 교정하면 현재 출시 상품 가격만 바꾸고 끝내지 않는다. `pointCost`, seed JSON, env 안내문, 테스트 기대값, 미래 후보 상품의 placeholder 가격까지 같은 경제 정책 축에서 함께 갱신해야 다음 승인/배포 단계에서 숫자가 엇갈리지 않는다.
- 2026-04-27: 이 워크스페이스에서 사용자가 `스테이징`이라고만 말하면 추가 확인 질문으로 멈추지 말고, 현재 `main`을 기준으로 스테이징 푸시와 Firebase staging 배포까지 한 번에 진행한다. 별도 확인은 사용자가 배포를 보류하거나 범위를 제한할 때만 한다.
- 2026-04-27: 보상몰에서 `최소 교환 포인트`와 `일/주/월 교환 한도`는 절대 같은 fallback 축으로 묶지 않는다. 최소값을 500P로 낮출 때 한도도 함께 500P로 붕괴하면 2000P 상품이 전부 막히므로, 한도는 별도 launch default와 env 값을 기준으로 계산하고 테스트로 분리해 둔다.
- 2026-04-27: 사용자가 브랜드/상품 이미지를 "실제 원본으로" 교정하면 생성 SVG나 임의 일러스트로 타협하지 않는다. 제공사 상품 이미지나 공식 사이트 로고처럼 출처가 분명한 자산으로 바로 교체하고, 원격 자산을 쓰면 CSP 허용 도메인과 seed 데이터까지 함께 맞춘다.
- 2026-04-27: 사용자가 여러 공식 자산 중 특정 로고 파일을 직접 골라주면, 이전에 고른 "공식 로고 후보"를 유지하지 말고 그 정확한 파일을 앱 자산으로 반영한다. 특히 브랜드 마크는 URL 출처보다 사용자가 승인한 시각 버전을 우선한다.
- 2026-04-27: 보상몰처럼 모바일 밀도가 높은 화면은 마케팅성 가이드 문구를 계속 쌓지 않는다. 사용자가 "줄이고 한 줄로" 교정하면 상단 카피, 가격 정보, 보관함 안내를 핵심 상태만 남기는 방향으로 압축하고, 클릭 가능한 바코드/삭제 같은 실제 행동을 먼저 드러낸다.
- 2026-04-27: mock 쿠폰 만료일을 고정 30일로 두지 않는다. 현재 판매 중인 상품이 60일 등 다른 유효기간이면 카탈로그/상품 메타데이터에서 만료일을 계산해 mock, provider 응답 fallback, 화면 표기가 모두 같은 기준을 쓰게 한다.
- 2026-04-27: 쿠폰 보관함처럼 실제 사용 맥락이 강한 화면은 상품 식별 정보와 사용 행동을 분리하지 않는다. 사용자가 바코드만 보고 헷갈리면 상품 이미지도 함께 두고, mock 쿠폰은 `테스트` 표식과 `사용 완료` 같은 정리 액션을 바로 노출해 실쿠폰과 구분되게 한다.
- 2026-04-27: 관제탑 운영 버튼 이름은 내부 구현이 아니라 실제 운영 행동을 그대로 써야 한다. 공급사 재조회인데 `보관함 재확인`처럼 사용자 화면 기준으로 부르면 운영자가 오해하므로, admin 액션은 `쿠폰 재조회`, `공급사 재확인`처럼 외부 처리 의미가 드러나게 맞춘다.
- 2026-04-27: 관리자 도구에서는 자유 입력 사유 prompt를 기본으로 붙이지 않는다. 실제 분기 로직 없이 로그용 문자열만 남기는 경우에는 고정 reason key를 코드에서 넘기고, 운영자는 즉시 실행 가능한 버튼만 보게 해야 흐름이 덜 끊기고 실수도 줄어든다.
- 2026-04-27: 저장 후 거의 바뀌지 않는 사용자 정보는 입력창을 계속 펼쳐 두지 않는다. 연락처처럼 재사용되는 값은 기본을 접힌 보기 모드로 두고, 마스킹된 값과 `수정` 버튼만 보여줘야 모바일 화면 밀도가 유지된다.
- 2026-04-27: 모바일 쿠폰 보관함에서는 상품 이미지를 주인공처럼 크게 두지 않는다. 상품 썸네일은 바코드/PIN보다 한 단계 낮은 보조 식별 요소로 작게 제한하고, 바코드 확대는 카드형 팝업보다 넓은 가로형 전체화면으로 보여줘야 실제 스캔 맥락에 맞다.
- 2026-04-27: 모바일 스캔 UX에서 "가로모드로 보여달라"는 요청은 단순히 넓은 카드가 아니라 회전된 랜드스케이프 표현을 뜻할 수 있다. 바코드 확대는 필요한 경우 90도 회전한 가로형 프레임으로 보여주고, 상품 이미지는 줄이더라도 해빛 마켓 카드와 위계가 어긋나지 않게 같은 기준 크기를 유지한다.
- 2026-04-27: changelog는 구현자 설명보다 사용자 체감 변화가 먼저 드러나야 한다. 내부 용어, 운영 버튼 이름, 메타 정리 같은 내용은 줄이고 "무엇이 더 쉬워졌는지" 중심으로 짧게 쓴다.
- 2026-04-27: 이전에 우회했던 인증 전략을 다시 바꿀 때는 "같은 브라우저라도 일반 탭/설치 앱/인앱 브라우저" 맥락을 먼저 구분해 설명한다. 특히 삼성 인터넷 Google 로그인처럼 redirect와 popup 모두 이력이 있는 문제는 단순히 "다시 redirect"라고 말하지 말고, 어떤 실패 모드를 피하려는 변경인지와 예전 실패가 재발하지 않는 근거를 함께 제시해야 한다.
- 2026-04-27: 삼성 인터넷 PWA 설치처럼 브라우저가 `beforeinstallprompt`를 주지 않는 실기기 증상이 확인되면, 대기 시간을 늘리는 식으로 "원클릭 설치"를 약속하지 않는다. 네이티브 설치 프롬프트는 브라우저 이벤트가 있을 때만 호출 가능하므로, 지원되지 않는 브라우저는 메뉴 기반 안내나 Chrome 설치 경로처럼 실제 가능한 선택지를 명확히 제공해야 한다.
...
- 2026-04-28: 자산 탭에서 포인트/HBT 숫자만 캐시하고 지갑 주소, 미니차트, 거래기록을 후속 쿼리에 맡기면 모바일에서는 부분 로딩 화면이 오래 남는다. 자산 화면은 지갑 주소와 미니차트, 거래기록도 같은 캐시 우선 원칙을 적용하고, timeout 결과가 기존 거래기록 캐시를 빈 로딩 상태로 덮어쓰지 못하게 해야 한다.
- 2026-04-28: 버전 문자열처럼 단순 치환이라도 Korean UTF-8 파일에는 PowerShell `Set-Content` 기본값을 쓰지 않는다. 인코딩이 깨지면 테스트가 대량 실패하고 앱 코드도 파싱되지 않으므로, 버전 bump는 인코딩 보존 도구나 명시적 UTF-8 옵션으로 처리하고 즉시 HTML 무결성/번들 검증을 돌려야 한다.
- 2026-04-28: 친구 연결 완료처럼 한 번만 보여야 하는 토스트는 localStorage만 믿으면 캐시 삭제 뒤 다시 살아난다. `notifications` 문서 자체에 `clientSeenAt` 같은 서버 쪽 소비 마커를 남기고, 오래된 `friend_connected` 문서는 토스트 없이 소비 처리해야 같은 연결 안내가 로그인마다 반복되지 않는다.
- 2026-04-29: 사용자용 해빛 마켓은 공급사 카탈로그 전체가 아니라 운영자가 공개하기로 정한 상품 allowlist만 렌더링해야 한다. 비즈머니 잔액, 발급 한도, 운영 기준 미달 같은 관제 정보는 스냅샷에 있더라도 사용자 UI에 직접 노출하지 말고 관제탑 화면에서만 확인하게 한다.
- 2026-04-29: 실제 기프티쇼 쿠폰 발급 테스트는 PIN을 노트에 남기지 않더라도 `TR_ID`, 상품코드, 대상 환경, 연결할 사용자 문서 같은 복구 키는 보안 범위 안에서 반드시 저장해야 한다. 공급사 상세 조회가 `TR_ID` 필수인 경우 이 키를 잃으면 이미 발급된 실쿠폰을 앱 보관함에 안전하게 복원할 수 없다.
- 2026-04-29: 보상몰의 비즈머니 운영 하한선은 관제/충전 알림 기준이지 사용자 교환 차단 기준이 아니다. 사용자 교환은 실제 공급사 잔액이 해당 상품 매입가보다 부족한 경우에만 막아야 하며, 운영 하한선 미달 메시지를 사용자 버튼 상태에 섞지 않는다.
- 2026-04-29: 사용자 보상몰 발급 정책을 바꾸면 관제탑도 같은 기준으로 즉시 맞춘다. 관리자 화면에 남은 가스/운영 예산, 운영 하한선 차단 문구 같은 과거 HBT 설계 흔적이 실제 쿠폰 교환 판단을 흐리게 만들 수 있다.
- 2026-04-29: AI 분석처럼 업로드된 미디어 URL을 읽어서 후속 데이터를 만드는 흐름은 분석 결과만 저장하면 안 된다. 사용자가 별도 저장을 누르지 않아도 재방문 시 같은 사진이 보여야 하므로, 분석 성공 patch에 해당 미디어 URL을 함께 넣고 일반 저장에서도 빈 슬롯 값이 기존 URL을 덮어쓰지 못하게 한다.
- 2026-04-29: 관제탑 보상마켓 요약은 운영자가 지금 결정을 내리는 데 필요한 항목만 남긴다. 준비금, 실발급 상태, 공개 상품, 재확인 동선이면 충분하고, 피드 소스/정산 자산/HBT 연동/누적 마진 같은 배경 정보는 별도 운영 문서나 상세 로그로 분리해야 실발급 판단을 흐리지 않는다.
- 2026-04-29: 기프티쇼 쿠폰 만료일은 seeded 카탈로그의 `validityDays`보다 공급사 발급/상태조회 응답의 `validPrdEndDt`를 우선한다. 특히 `result[0].couponInfoList[0]`처럼 중첩된 응답을 놓치면 30일 실쿠폰이 60일 fallback으로 저장될 수 있으므로, `YYYYMMDDHHmmss`는 한국시간 만료시각으로 파싱해 저장한다.
- 2026-04-29: 쿠폰 보관함의 유효기간은 날짜만 보여주지 말고 한국시간 날짜 기준 남은 일수를 함께 보여준다. 실쿠폰은 만료일 오해가 곧 사용 실패로 이어질 수 있으므로 `까지` 같은 막연한 표현보다 `30일 남음`처럼 행동 판단에 바로 쓰이는 문구를 우선한다.
...
- 2026-06-02: 사용자가 여러 소모임 참여를 허용하자고 한 경우, 중복 악용 방지는 가입 개수/승인 흐름으로 제어하고 보상 진행도는 사용자가 가입한 각 소모임 단위로 따로 오른다고 먼저 가정한다. “같은 날짜 최대 1회” 같은 전역 제한은 참여 선택지를 무의미하게 만들 수 있으므로 제품 의미를 확인한 뒤 적용한다.
- 2026-06-02: 대시보드 반복 카드에는 “탭 한 번으로”나 “기록을 남기면” 같은 사용법 설명을 기본 노출하지 않는다. 사용자가 이미 기능을 이해하는 운영형 화면에서는 진행도와 상태 숫자를 한 줄로 압축하고, 추천 정렬처럼 실제 선택 판단에 도움이 되는 신호를 우선한다.
- 2026-06-17: 챌린지 진행도는 저장을 실행한 현재 시간이 아니라 `daily_logs` 문서의 대상 날짜를 기준으로 반영해야 한다. 저장 UI가 이전 날짜를 선택할 수 있으면 후속 집계 함수에도 `selectedDateStr`와 방금 저장한 log 데이터를 넘겨, 다음날 올린 전날 기록이 포인트만 받고 챌린지에서는 빠지는 일을 막는다.
- 2026-06-17: 챌린지 시작/복구 서버 기록이 성공한 뒤 화면 새로고침이 실패해도 전체 시작을 실패로 뒤집지 않는다. `updateAssetDisplay(true)`는 별도 helper에서 catch하고 지연 재조회까지 예약해, 사용자가 앱을 닫았다 열어야만 챌린지가 보이는 상태를 만들지 않는다.
...
- 2026-06-28: 챌린지 보상 수령처럼 서버 mutation이 성공한 뒤 `users` 문서 재조회가 늦어질 수 있는 화면은 제목/설명만 남은 빈 카드로 두지 않는다. 완료/수령 후에는 공용 서버 우선 refresh와 지연 재확인을 쓰고, 사용자 문서가 timeout되면 안전한 "확인 중" 상태를 렌더해 캐시 숫자와 챌린지 본문이 서로 어긋나 보이지 않게 한다.
- 2026-07-12: 개인정보 경계를 강화할 때 앱의 핵심 콘텐츠까지 통째로 제거하지 않는다. 식단 AI 분석처럼 사용자가 공유하기로 한 미디어에 붙는 핵심 결과는 서버가 표시용 필드만 allowlist로 재구성하고, 공유 사진이 있는 슬롯에만 연결해 원문·해시·알 수 없는 필드는 계속 차단한다.
- 2026-07-12: 전체 루틴 목표와 카테고리 최대점수는 다른 완료 기준이다. 합계 65P로 풀 루틴을 달성했다면 각 카드의 실제 점수 배지는 유지하되 모든 루틴 카드를 완료 색으로 보여 주고, 64P 경계를 회귀 테스트로 함께 고정한다.
- 2026-07-12: 첫 저장 결과처럼 한 번만 뜨는 화면에도 가입 선물이나 알림 권한 제안을 자동으로 쌓지 않는다. 기존 회원 노출 여부와 1회 마커를 먼저 확인하고, 사용자가 다음 행동을 고르는 핵심 CTA만 남겨 staging 실기기에서 문구와 밀도를 검증한다.
- 2026-07-12: 자산 탭을 단기 쿠폰 전환 중심으로 단순화할 때 HBT 잔액과 전환까지 `고급 기능`으로 접지 않는다. 포인트 쿠폰과 HBT 보유는 서로 다른 핵심 가치이므로 두 잔액·활성 체인·전환 동선은 상단에 유지하고, 챌린지·채굴 정보·지갑 기술 세부만 점진적으로 공개한다.
- 2026-07-12: HBT 전환을 눈에 띄게 복구할 때는 화면만 열어 두지 말고 지갑이 없는 신규 회원의 실제 전환 경로도 함께 검증한다. 기존/외부 지갑을 먼저 복원하고, 신뢰할 수 있는 서버 문서에서 지갑 부재를 재확인한 뒤 Firestore transaction의 compare-and-set으로 한 지갑만 생성해야 한다. 단일 탭 Promise만으로는 여러 탭·기기의 동시 생성을 막을 수 없으며, 전환 callable 전에도 최종 지갑 준비 완료를 확인한다.
- 2026-07-12: 클라이언트와 Cloud Function 사이의 쓰기 계약을 바꾼 릴리스는 Hosting만 먼저 본서버에 배포하지 않는다. `awardedPoints`처럼 클라이언트 쓰기를 제거하고 서버 원장으로 넘기는 변경은 Functions·Firestore/Storage rules까지 하나의 호환 단위로 배포하고, 배포 직후 실제 트리거 로그와 서버 확정값을 확인해야 한다. 낙관 계산값은 서버 ACK·정산 전에는 확정 포인트나 완료 상태로 영구 캐시하지 않는다.

## 2026-07-13 점수 낙관 UI와 관제탑 조회 교훈

- 클라이언트와 서버는 같은 포인트 규칙을 사용하되, 보안 강화를 이유로 확인되지 않은 제품 규칙을 임의로 축소하지 않는다. 해빛스쿨에서는 사용자가 입력한 8,000보 자체가 유산소 1회이며 캡처는 필수 조건이 아니다. 사진 1장과 8,000보가 함께 있으면 15P가 되어야 한다.
- 로컬 미디어 미리보기 보존과 서버 확정 점수 hydration은 분리한다. 미디어 DOM을 건너뛰더라도 확정 `awardedPoints`는 항상 화면을 덮어써야 한다.
- 관리자 첫 화면에서 큰 Firestore 컬렉션을 브라우저가 여러 차례 직접 스캔하지 않는다. 관리자 권한을 확인하는 서버 집계 호출로 최소 결과만 반환하고, fallback 조회가 필요하면 모든 독립 쿼리를 동시에 시작해 timeout 워터폴을 막는다.
- 포인트 정책을 변경하면 새 코드만 맞추고 끝내지 않는다. 정산 정책 버전을 올려 기존 완료 문서가 다음 쓰기에서 재평가되게 하고, 이미 누적된 불변 원장에는 차액만 추가되는 경로를 경계값과 함께 검증한다.

## 2026-07-14 관제탑 서버 스냅샷 크기 교훈

- 선택적 숫자 override에서 `Number(null)`은 `0`이 되므로, nullish 기본값을 유효한 숫자처럼 검사하면 실제 행이 있어도 집계 크기가 0으로 보인다. 숫자 변환 전에 `null`/`undefined` 존재 여부를 분리하고, override가 없으면 실제 배열 길이를 사용한다.
- 서버가 반환한 관리자 요약을 Firestore 스냅샷 모양으로 어댑트할 때는 `forEach()` 데이터뿐 아니라 `size`도 같은 원본을 가리키는지 회귀 테스트한다. TOP 목록과 총계 카드가 서로 다른 값을 보여도 서버 데이터 손실로 단정하지 않는다.

## 2026-07-16 풀 루틴 배지 교훈

- 달성 기준과 표시 점수는 분리한다. 풀 루틴 완료 기준은 65P로 유지하되, 기준을 넘겨 75P를 얻었다면 완료 배지에도 실제 합계 75P를 표시해야 추가 기록의 성취감이 사라지지 않는다.
- 완료 색상만 실제 합계에 맞추고 숫자를 기준값으로 고정하지 않는다. 64P, 65P, 기준 초과 점수를 함께 테스트해 판정과 표시가 각각 올바른지 확인한다.

## 2026-07-17 공급사 성공 후 쿠폰 저장 실패 교훈

- 공급사 날짜 필드는 ISO 문자열만 온다고 가정하지 않는다. 10자리 epoch seconds, 13자리 epoch milliseconds, 공급사 전용 8/14자리 날짜를 먼저 정규화하고 Firestore의 1~9999년 범위를 검증한 값만 Timestamp로 쓴다.
- 쿠폰 주문은 공급사 성공과 앱 저장 성공을 한 결과로 취급하지 않는다. 최종 저장이 실패해도 같은 거래 ID 상태조회로 복구하고, 미해결 같은 상품 주문이 새 거래 ID로 다시 발급되지 않게 막는다.
- 재발송은 신규 주문의 대체물이 아니다. 기존 거래 ID와 저장된 연락처만 사용하고 본인 확인, 서버 트랜잭션 선점, 쿨다운·일일 제한, 감사 원장을 함께 적용한다.
- 관제 화면의 발급 이력은 전체 회원·기록 스캔에 종속시키지 않는다. 발급 문서에 저장된 사용자 라벨로 핵심 표를 먼저 렌더하고 준비금·가격·카탈로그는 독립 timeout으로 보강한다.

## 2026-07-18 쿠폰 표시와 재발송 상태 교훈

- 공급사 재발송 API의 성공 코드는 요청 접수 성공이지 단말 도달 완료가 아니다. `requestedAt`과 실제 `deliveredAt`을 분리하고, 배달 상태를 확인하지 못한 화면에서는 “발송 완료”라고 안내하지 않는다.
- HTTPS 앱에서 공급사가 HTTP 쿠폰 이미지 URL을 반환할 수 있다. PIN이 있으면 혼합 콘텐츠나 이미지 로드 실패를 깨진 이미지로 노출하지 말고 앱 내부 바코드로 즉시 대체한다.
- 공급사 유효기간 필드는 누락되거나 형식이 달라질 수 있다. 상품 카탈로그의 유효기간을 저장 시 fallback으로 쓰고, 기존 누락 기록은 발급일 기준으로 복원하되 화면에 상품 기준 추정값임을 표시한다.

## 2026-07-18 운동 영상 미리보기와 기존 쿠폰 복구 교훈

- 선택 직후 운동 영상은 업로드·썸네일 추출 완료를 기다리지 않는다. 대용량·다중 선택에서는 FIFO 순서 때문에 검은 플레이스홀더가 오래 남으므로 로컬 object URL의 실제 영상 프레임을 먼저 보여 주고, 카드 삭제·교체 시 URL을 반드시 revoke한다.
- 공급사 응답 파싱을 고친 것만으로 이미 공급사 성공·Firestore 미완료 상태에 남은 쿠폰은 회복되지 않는다. 외부 거래 ID가 있는 `pending_issue` 문서는 재주문이나 재차감 없이 상태 조회만 수행하는 소유자 전용 복구 경로를 함께 제공한다.
- 쿠폰 보관함의 기본 스냅샷을 공급사 조회에 묶지 않는다. 기존 보관함은 먼저 렌더하고 미해결 쿠폰만 백그라운드에서 순차 확인해, 공급사 지연이 전체 자산 탭 로딩 지연으로 번지지 않게 한다.

## 2026-07-18 저장 완료 후 운동 영상 썸네일 교훈

- 선택 직후 실제 영상이 보이는 것만으로 운동 영상 미리보기가 해결됐다고 판단하지 않는다. 저장 완료 렌더, 썸네일 지연 완료, 새로고침 복원까지 같은 영상 카드의 전체 생애주기를 검증한다.
- 브라우저가 이미 화면에 디코딩한 로컬 영상 프레임은 별도 숨은 video 디코더보다 신뢰도가 높다. 그 프레임을 축소 JPEG로 캡처해 썸네일 업로드에 재사용하고, 영구 썸네일이 준비되기 전에는 정상 로컬 프레임을 검은 fallback으로 교체하지 않는다.
- 상태의 정확성을 설명하는 보조 문구라도 모바일 한 줄 정보를 깨뜨리면 핵심값 인지가 나빠질 수 있다. 쿠폰 유효기간처럼 값 자체가 충분한 경우 추정 근거 문구는 화면 제목에서 빼고 운영 데이터에만 유지한다.

## 2026-07-19 기존 주간 미션 중심 건강 실천 연결 교훈

- 목표 실행 흐름을 개선할 때는 별도 목표 모델보다 현재 저장 기준을 먼저 정한다. 해빛스쿨에서는 `weeklyMissionData`를 기준으로 두고 `primaryHabit`은 첫 분류 선호, `selectedMissions`는 호환 읽기로만 제한해야 중복 기능을 만들지 않는다.
- 리포트 안내는 실제 조회·계산 범위보다 넓게 단정하지 않는다. 최신 30개 기록만 읽는 결과지에는 “지난 30일 선택 미션” 대신 “최근 기록의 실천”처럼 현재 데이터가 증명하는 범위만 표현한다.
- 제품 이벤트는 정의 여부보다 실제 의미가 발생한 성공 지점과 중복 방지가 중요하다. 기록 저장 뒤에만 판정하고, 허용된 버킷만 전송하며, 전송 성공 뒤에만 로컬 마커를 남겨 실패 시 재시도를 보존한다.

## 2026-07-19 Firebase Storage 영상 CORS 교훈
...
## 2026-07-20 분리 CSS와 관리자 기록 재사용 교훈

- CSS 파일을 분리할 때는 선택자뿐 아니라 파일 경계에 걸린 `@media` 시작·종료 괄호도 함께 옮긴다. 브라우저가 구문 오류를 조용히 복구하면 다음 정상 규칙까지 버릴 수 있으므로, 화면 증상만 보고 레이아웃을 재설계하지 말고 계산 스타일과 CSS 규칙 파싱 여부를 먼저 확인한다.
- 관리자 상세의 AI 결과는 새 분석을 호출하지 않고 개인 기록에 이미 저장된 결과의 허용 필드만 이스케이프해 보여 준다. 원문 JSON, 프롬프트, 해시, 알 수 없는 필드는 운영 화면에도 그대로 노출하지 않는다.
- 특정 회원의 최근 기록은 전체 최신 N건 캐시를 다시 필터링하지 않는다. 기존 인덱스를 사용한 회원·날짜 범위 쿼리로 읽어야 전체 사용량이 커져도 해당 회원 기록과 저장된 미디어가 누락되지 않는다.
- 관리자 미디어 확대는 표시할 때와 클릭할 때 Storage URL을 각각 검증하고, 영상까지 지원하면 허용 출처를 좁힌 `media-src` CSP를 함께 확인한다.

## 2026-07-22 주간 미션 기본값·삼성 선택기·A4 2-up 교훈

- 선택 화면의 기본값을 바꿀 때는 저장된 상태를 다시 렌더하는 분기와 신규 선택 상태를 분리한다. 이번 주 활성 `weeklyMissionData`는 그대로 두고 비활성 선택기의 체크 속성만 바꿔야 기존 미션과 난이도를 덮어쓰지 않는다.
- 웹/PWA의 `<input type="file">`는 Android 작업 선택에서 특정 앱이나 `파일` provider를 자동 선택할 수 없다. `accept`는 파일 형식, `capture`는 카메라 방향 힌트일 뿐이므로 네이티브 선택 경로를 우회하지 말고 선택 전에 보이는 기기별 안내를 제공한다.
- 화면 DOM을 그대로 인쇄하면 `body` flex 같은 화면 레이아웃과 긴 canvas가 페이지 분할에 개입한다. 인쇄 전용 DOM으로 격리하고 canvas는 이미지 스냅샷으로 복제하며, 실제 PDF의 페이지 수와 렌더 이미지를 함께 확인한다.
- 2-up 인쇄에서 콘텐츠 폭을 역비례로 넓혀 축소할 때 종횡비 기반 그래프와 히트맵은 물리적 높이가 줄지 않을 수 있다. 인쇄 전용 차트·셀 높이를 먼저 고정해야 실측 scale이 잘림을 실제로 해소한다.

## 2026-07-23 Android 30일 결과지 인쇄 수명주기 교훈

- Android/Samsung의 `afterprint`는 네이티브 인쇄 미리보기가 실제 DOM을 캡처한 뒤라는 보장이 없다. 이벤트에서 인쇄 전용 DOM과 상태 클래스를 즉시 제거하면 미리보기가 스크롤 중인 화면 모달을 뒤늦게 캡처해 상단 누락, 중복, 페이지 증가가 발생할 수 있다.
- 인쇄 전용 DOM은 화면에서는 숨긴 채 결과지 닫기 또는 다음 인쇄 준비처럼 사용자가 통제하는 시점에 정리한다. 인쇄 미리보기에 화면용 버튼이 보이면 축소 CSS보다 먼저 전용 DOM 수명주기가 끊겼는지 확인한다.
- 정적 인쇄 시트만 PDF로 검증해서는 화면 DOM으로 되돌아가는 회귀를 잡지 못한다. 화면용 원본 모달을 같은 문서에 함께 둔 검증 fixture로 A4 1페이지, 첫 면 헤더, 두 면의 단일 노출, 화면용 액션 미출력을 함께 확인한다.

## 2026-07-23 Staging 배포 승인 교훈

- 사용자가 `스테이징` 또는 `staging`이라고 명시하면 staging Hosting 및 Functions 배포의 최종 승인이다. 커밋·푸시가 필요하면 먼저 완료하되, 푸시 뒤 같은 승인을 반복해서 요청하지 말고 두 대상의 배포와 검증까지 이어서 완료한다.
- 이 단축 승인을 운영 서버나 Firestore·Storage 규칙·인덱스 배포로 확대하지 않는다.

## 2026-07-23 관제탑 권한·갤러리·쿠폰 증빙 교훈

- 관제탑 화면에서 다른 회원 문서를 직접 쓰기 전에 Firestore 규칙과 반드시 대조한다. 교차 사용자 쓰기는 관리자 callable에서 권한·입력 검증과 감사 기록을 함께 처리하고, 클라이언트 권한을 넓히지 않는다.
- 공급사 API 요청 접수, 문자 발송 확인, PIN 발행, 앱 PIN·이미지 보관은 서로 다른 증빙이다. 하나만 성공했다고 전체를 `발급 완료`로 표시하지 말고 상태와 안내 문구를 증빙별로 분리한다.
- 공급사 응답 필드는 이름만 보고 주문번호로 추정하지 않는다. 실제 운영 응답에서 `sendRstCd` 같은 결과 코드와 식별자를 구분하고, 경계 응답을 fixture로 고정한다.
- 모달을 닫을 때는 내부 포커스를 원래 트리거로 먼저 돌린 뒤 부모를 숨기거나 `aria-hidden`으로 전환한다. 보이는 기능이 정상이어도 접근성 경고는 포커스 순서 회귀의 신호다.
- 현재 저장소에 복구 코드가 있어도 운영 Functions revision에 배포됐다는 뜻은 아니다. 쿠폰 같은 외부 거래 흐름은 배포 함수 목록·revision·실패 로그까지 확인한 뒤 정상이라고 판단한다.

## 2026-07-23 운영 초기 데이터 재시도 교훈

- 여러 하위 조회가 하나의 화면을 채울 때 재시도 횟수는 누락 신호 개수가 아니라 실제 재요청 실행 횟수로 계산한다. 같은 로드에서 여러 누락이 발생하면 기존 2초 타이머를 유지하고 하나의 재요청으로 합쳐야 한다.
- 캐시 화면 렌더 성공과 권위 데이터 갱신 성공은 다르다. cache-only 화면을 표시한 뒤 예약한 최신 조회를 공통 렌더 완료 코드가 무조건 취소하면 앱 재진입 전까지 오래된 상태가 고착된다.
- 핵심 화면의 독립 데이터 소스는 하나의 캐시 조기 반환에 함께 묶지 않는다. 자산 기본값이 캐시에 있어도 마켓·쿠폰은 별도로 갱신하고, 실패한 소스만 2초 간격 최대 3회 재시도한다.

## 2026-07-31 안드로이드 아이콘 자산 교훈

- `<bitmap android:gravity="center">`는 비트맵 크기를 조정하지 않는다. 512px 아이콘을 108dp 어댑티브 전경으로 얹으면 캔버스를 넘쳐 가운데만 남고, `inset` 값을 아무리 바꿔도 표시 창만 좁아질 뿐 같은 증상이 반복된다. 스케일이 필요하면 `gravity="fill"`을 쓰고, 여백은 이미지 자체에 넣는다.
- 어댑티브 아이콘 전경은 108dp 중 바깥 18dp가 마스크에 잘릴 수 있다. 안전 영역 안에 있는지는 눈으로 보지 말고 PNG 알파의 바깥 띠가 비어 있는지 테스트로 고정한다.
- 앱 아이콘(배경·문구 포함 정사각형)과 런처 전경 레이어는 다른 자산이다. 하나를 돌려쓰지 말고 생성 스크립트(`icons/generate_app_icons.py`)로 각각 만든다.
- 웹푸시 `badge`(안드로이드 알림 작은 아이콘)는 알파 채널만 읽어 실루엣으로 그린다. 불투명 컬러 PNG를 주면 알림 줄에 흰 사각형만 보인다. 투명 배경 + 단색 실루엣 전용 이미지를 쓴다.
- 런처 아이콘 수정은 호스팅 배포로 반영되지 않는다. versionCode를 올린 새 AAB를 플레이 콘솔에 올려야 설치된 앱에 적용된다.

## 2026-08-06 공급사 유효기간 필드 오독 교훈

- 공급사가 `limitDay` 같은 "일수" 필드에 종료일(YYYYMMDD)을 넣어 보낼 수 있다. 8자리 값을 일수로 믿으면 만료일이 서기 5만 년으로 튀고 Firestore Timestamp 범위를 넘겨 발급 기록 커밋이 통째로 실패한다. 날짜 모양이면 잔여일로 환산하고, 일수 후보는 상식 범위로 클램프한다.
- 방어 함수가 fallback을 검증하지 않으면 방어가 아니다. `toSafeFirestoreDate`가 값만 검사하고 fallback을 그대로 통과시켜, 같은 오염값이 fallback 자리로 우회해 commit까지 내려갔다.
- 외부 발급이 성공한 뒤의 저장 실패는 사용자에게 "교환 오류"로 보이지만 실제로는 쿠폰이 이미 발급·발송된 상태다. 발급 호출 이후 코드는 어떤 예외도 기록을 잃지 않도록 취급하고, 갇힌 문서를 되살릴 경로를 함께 확보한다.
- 표시 기능이 안 보인다고 UI가 지워졌다고 단정하지 않는다. 이번 건은 렌더러가 멀쩡했고 저장된 데이터가 비어 있었다. 화면 문구가 어느 분기에서 나오는지부터 역추적하면 서버/클라이언트를 빨리 가른다.
- 공급사 상태조회(0201)가 성공 코드를 주면서도 PIN·이미지를 안 줄 수 있다. 값 없이 필드 이름만 로그로 남겨 두면 "공급사가 안 준 것"과 "우리가 못 읽은 것"을 다음 조회에서 바로 구분할 수 있다.
- mixed content 방어로 공급사 이미지를 차단할 때는 대체 경로를 함께 준비한다. PIN까지 비어 있으면 방어가 곧 기능 소실이 된다. 발급 시점에 Storage로 미러링해 HTTPS 사본을 쓰면 방어를 유지하면서 화면도 살릴 수 있다.

## 2026-08-06 느린 발급과 클라이언트 timeout 교훈

- Firebase callable의 클라이언트 기본 timeout은 70초다. 함수에 `timeoutSeconds: 180`을 줘도 클라이언트가 먼저 끊으면 `deadline-exceeded`가 뜨고, 실제로는 성공 중인 작업이 실패로 보인다. 서버 timeout을 늘릴 때는 `httpsCallable(fn, name, { timeout })`도 같이 맞춘다.
- 외부 API가 낀 흐름에서 준비 조회를 직렬 `await`로 늘어놓지 않는다. 서로 의존하지 않는 조회는 `Promise.all`로 묶는다. 이번 건은 가격·준비금·비즈머니·한도·공급사 카탈로그(최대 3000건) 5개가 직렬이라 콜드 스타트에서 75초가 나왔다.
- 모듈 레벨 캐시는 콜드 스타트마다 비어 있다. 15분 캐시가 있어도 첫 호출은 항상 전체 왕복을 낸다는 전제로 지연을 계산한다.
- 몇십 초 걸리는 작업은 진행 상태를 보여 주지 않으면 사용자가 실패로 읽고 같은 버튼을 반복해 누른다. 남은 시간을 표시하고 진행 중 버튼을 잠근다. 멱등 키가 있어 이중 결제는 막혔지만, 그건 방어지 UX가 아니다.

## 2026-08-20 스테이징 확인 없이 본서버에 올린 건

- **스테이징에 올린 것과 스테이징에서 확인받은 것은 다르다.** 스테이징 v326을 올리며 라이트 챌린지와 친구 알림 확인을 요청해 놓고, 확인 답이 오기 전에 "본서버"라는 말만 보고 배포했다. 확인 요청을 한 쪽이 나였으므로, 그 답이 오기 전까지는 배포 조건이 성립하지 않는다.
- 사용자가 "본서버"라고 말해도, **내가 아직 확인을 기다리고 있는 항목이 있으면 그건 확인 완료 신호가 아니다.** 무엇을 확인받았는지 먼저 되짚고, 안 받았으면 그 사실을 말한 뒤 기다린다.
- **본서버와 스테이징의 버전은 벌어진 채로 두지 않는다.** 스테이징이 앞서는 것은 검증 대기 중일 때뿐이고, 검증이 끝나면 곧바로 본서버를 같은 버전으로 맞춘다. 검증도 안 됐는데 벌어져 있으면 어느 쪽이 기준인지 알 수 없어진다.
- 배포 후에는 두 환경의 스탬프를 나란히 찍어 확인한다. 한쪽만 보면 `hosting,functions` 묶음에서 hosting이 빠지는 것 같은 누락을 놓친다(이번에 실제로 발생).

## 2026-08-21 쓰기 경로를 옮기고 집계를 안 옮긴 건

- **데이터의 쓰기 경로를 바꿀 때는 그 데이터를 읽는 곳을 전부 같이 옮긴다.** 리액션 UID 위조 취약점을 막으려고 쓰기를 `daily_logs.reactions` → `gallery_posts` 로 옮겼는데, 집계 세 곳은 계속 `daily_logs` 를 읽었다. 한 달 내내 화면의 리액션이 0으로 표시됐고(실제 494개) MVP 점수가 `days×10` 하나로 붕괴했다. 보안 수정은 성공했고 그 옆의 집계가 조용히 죽었다.
- **같은 식이 세 벌 복사돼 있으면 언젠가 갈라진다.** 점수 식이 지급·시간별 집계·아카이브에 각각 있었다. 셋을 한 모듈로 모으고, 다시 복사되는 것을 테스트로 막았다. 화면과 실제 지급이 다른 순위를 쓰는 건 버그가 아니라 사고다.
- **정렬 키가 하나면 동점은 무작위다.** `sort((a,b) => b.score - a.score)` 뿐이면 동점자 순서가 Firestore 스캔 순서를 따른다. 8월엔 210점 동점자가 6명이었고 5,000P가 그 순서로 나갈 뻔했다. 돈이 걸린 정렬에는 uid까지 내려가는 결정적 tie-break를 반드시 넣는다.
- **0은 "값이 없다"가 아니라 "읽는 곳이 틀렸다"의 신호일 수 있다.** 커뮤니티 통계의 댓글·리액션이 0으로 굳어 있었는데 아무도 이상하게 보지 않았다. 집계 지표가 정확히 0으로 오래 머물면 데이터가 아니라 쿼리를 의심한다.
- **지표가 포화됐다는 신고를 받으면 지표를 바꾸기 전에 그 값이 맞는지부터 잰다.** 이번엔 "1·2·3위 변별력이 없다"는 신고였고 기준 변경만 했으면 버그를 안은 채 새 식을 얹을 뻔했다.
- **설계를 바꾸자는 제안은 표본부터 잰다.** 주간 리스트를 공유·초대 기준으로 바꾸자는 안은 합리적이었지만, 회원 579명 중 초대 유입이 17명(2.9%)이고 8월 초대 성공이 0건이었다. 순위를 만들 사람이 없으면 가중치 논의는 의미가 없다. 랭킹 설계 문제가 아니라 유입 설계 문제였다.

## 2026-08-21 쿼터 429가 hosting 을 통째로 떨어뜨린 건

- **`--only hosting,functions` 는 원자적이지 않다.** 함수가 40개를 넘으면 `Per project mutation requests per minute per region` 쿼터(429)에 걸리고, 재시도로 함수는 결국 다 올라가지만 **hosting 은 배포되지 않은 채 명령이 끝난다.** 이번에 프로덕션이 v330 에 그대로 남아 있었다. 2026-08-20 에 같은 누락을 겪고 "스탬프를 나란히 찍어 확인한다"고 적어둔 덕에 잡았다.
- **배포 로그의 성공 줄을 세지 말고 결과를 직접 조회한다.** `+ Successful update operation` 이 수십 줄 흘러도 hosting 은 그 목록에 없다. `curl -s <url>/sw.js | grep habitschool-v` 로 프로덕션과 스테이징 스탬프를 나란히 찍는 게 유일하게 믿을 수 있는 확인이다.
- **함수가 많은 프로젝트는 `--only functions` 와 `--only hosting` 을 나눠 올리는 편이 낫다.** 묶으면 쿼터에 걸린 쪽이 안 걸린 쪽까지 끌고 내려간다.
- **`functions:list` 는 새 코드가 올라갔는지 알려주지 않는다.** 함수 이름과 런타임만 나온다. 배포 확인은 그 코드가 실제로 만들어내는 값으로 한다 — 이번엔 집계를 한 번 돌려 `totalReactions` 가 0에서 494로 바뀌는 것을 봤다.

## 2026-08-26 고친 것과 나간 것 — 네이티브 리소스 편

- **AAB를 올렸는지는 커밋 로그로 확인되지 않는다.** 7/31 에 런처 아이콘 crop 을 고치고
  versionCode 5 까지 올려 AAB 까지 빌드해 놓고 **업로드를 안 했다.** 트랙은 8/26 까지
  버전 4 에 머물렀고, "이 출시 버전을 설치한 사용자 비율 100%" 였으니 테스터 전원이
  한 달간 아이콘 깨진 빌드를 썼다. 8/5 리마인드 메일에는 이미 "다음 업데이트에
  반영됩니다"라고 적어 보낸 뒤였다.
- **호스팅 배포로 나가는 수정과 AAB 로만 나가는 수정을 섞어 세지 않는다.** 같은 기간의
  로그인·챌린지·업로드·갤러리 수정은 웹이라 이미 테스터에게 갔다. TWA 셸 안의
  리소스(런처 아이콘, 모노크롬 레이어)만 안 갔다. "다 고쳤다"가 "다 나갔다"로 읽히는
  지점이 정확히 여기다. `firestore.rules` 를 파일에만 고쳐 두었던 2026-08-15 건과 같은
  모양이다.
- **빌드 산출물 폴더에 오래된 파일이 남아 있으면 사고 자재가 된다.**
  `apk/release/app-release.apk` 는 4월 빌드(versionCode 2 / targetSdk 35)인데 최신
  AAB 옆에 그대로 놓여 있었다. 업로드 직전에 손에 잡히는 파일을 집으면 그대로 거부된다.
  올릴 파일은 매니페스트를 열어 versionCode·targetSdk 를 확인하고 집는다.
- **Play Console 경고 배너는 오류와 같은 모양으로 뜬다.** "버전에 문제가 있습니다"를
  펼치면 난독화 매핑 파일 없음(경고 1개)뿐이었다. `isMinifyEnabled = false` 인 프로젝트에
  없는 파일을 안 올렸다는 안내다. 배너 문구가 아니라 펼친 내용의 등급을 본다.
- **콘솔 알림은 해결돼도 지워지지 않는다.** 7/30 자 targetSdk 경고가 남아 있었지만
  트랙의 버전 4·5 는 이미 타겟 SDK 36 이었고, 개발자 인증도 3/13 에 등록 완료였다.
  알림을 보고 움직이기 전에 해당 화면에서 현재 값을 직접 확인한다.

## 2026-08-27 제보가 원인을 못 담아 온 건

- **에러 코드를 삼키면 제보는 "실패했대요"가 전부가 된다.** 식단 AI 분석이 "껐다 켜야
  된다"는 제보를 받았는데, 화면 문구가 고정된 한 문장이라 **토큰 만료·콜드스타트
  타임아웃·방금 올린 이미지를 서버가 못 읽음** 셋 중 무엇인지 가릴 수가 없었다. 셋 다
  "재시작하면 된다"와 맞는다. 코드를 토스트에 실어 보내면 다음 제보 한 건으로 갈린다.
  CLAUDE.md 에 이미 적어둔 원칙인데 같은 파일에서 네 번 반복되고 있었다.
- **증상 설명과 화면 문구가 다르면 문구부터 따라간다.** "저장 전에 눌러서 그런 것
  아니냐"는 그럴듯한 가설이었지만, 저장 전 경로에는 이미 별도 가드와 **다른 메시지**가
  있었다(`사진을 먼저 저장한 후 분석해주세요`). 제보 화면의 문구는 서버 호출까지 갔다가
  실패한 경로에서만 나온다. 가설대로 버튼을 비활성화했으면 엉뚱한 곳을 고칠 뻔했다.
- **`?v=` 버전을 올릴 때 `styles.css` 를 빼먹기 쉽다.** `*.js`/`*.html` 만 훑으면
  안 걸린다. CSS 안에 `@import url('./styles-base.css?v=336')` 가 일곱 줄 들어 있다.
  `tests/pwa-versioning.test.js` 가 잡아줬다 — 이런 테스트가 있는 이유다.
- **배포 직후 curl 확인은 자기 캐시에 속는다.** `--only hosting` 이 성공했는데 스탬프가
  옛 버전으로 찍혔다. 쿼리스트링(`?cb=$RANDOM`)을 붙이자 새 버전이 나왔다. 2026-08-21 에
  적어둔 "스탬프를 나란히 찍는다"에 **캐시 버스터를 붙인다**를 더한다.
- **소스 텍스트 테스트는 실행을 보장하지 않는다.** 새 테스트 9건이 통과했지만 그건
  문자열 검사였다. ESM 문법 오류였다면 앱 전체가 안 떴을 것이다. 스테이징에서 모듈을
  실제로 import 해 보고, 새 i18n 키가 키 이름 그대로 나오지 않는지까지 확인해야 끝이다.

## 2026-08-30 LE8 건강습관 점수 — 도구가 조용히 부순 것

- **Python `io.open(p,'w')` 는 윈도우에서 파일 전체의 줄바꿈을 LF→CRLF 로 바꾼다.**
  `styles-reports.css` 에 28줄을 더했을 뿐인데 `report-print-layout.test.js` 가 깨졌다.
  그 테스트는 `'@page {\n    size: A4 portrait;'` 처럼 **`\n` 을 박아 비교**하는데,
  내가 손대지 않은 줄까지 전부 `\r\n` 이 되어 있었다. `git diff` 는 CRLF 를 정규화해
  **"28줄 추가, 0줄 삭제"** 로만 보여줘서 진단이 늦어졌다. 텍스트 파일을 통째로 다시 쓸
  땐 바이너리 모드(`'rb'`/`'wb'`)로 읽고 쓰거나 `newline=''` 을 준다.
- **`git diff --numstat` 이 깨끗하다고 파일이 멀쩡한 건 아니다.** 줄바꿈·BOM 변화는
  git 정규화에 가려진다. 의심되면 `file -b <경로>` 로 실제 바이트를 본다.
- **`cat > 파일` 은 이미 있는 파일을 묻지 않고 지운다.** 미리보기 서버를 띄우려고
  `.claude/launch.json` 을 새로 썼는데, 거기엔 이미 Firebase Hosting·에뮬레이터·Vitest·
  Hardhat 설정 5개가 들어 있었다. 게다가 내가 필요했던 로컬 정적 서버는 그중
  `Firebase Hosting (로컬 서버)` 로 **이미 있었다**. 추적되는 설정 파일에 쓰기 전에
  먼저 읽는다. `git status` 에 `M`(수정)으로 뜨면 새로 만든 게 아니라 덮어쓴 것이다.
- **새 JS 모듈은 `sw.js` 프리캐시 목록에도 넣어야 한다.** 안 넣으면 오프라인에서만
  깨진다. `tests/pwa-offline-assets.test.js` 가 import 그래프를 따라가며 잡아줬다.
- **규칙 파일을 고치기 전에 정말 고쳐야 하는지 확인한다.** 계획서에는 `firestore.rules`
  화이트리스트에 `heightCm`·`smokingStatus` 를 추가하고 배포하라고 적었지만, 실제로는
...
## 2026-08-30 배포하고 스탬프를 안 맞춘 건

- **본서버만 올리고 스테이징을 두고 오면, 다음 검증이 낡은 코드를 상대로 이뤄진다.**
  v342 를 운영에만 배포하고 스테이징을 341 로 남겼다. 둘 다 각자는 정상으로 보이므로
  드러나지 않는다. 2026-08-20 에 "벌어진 채로 두지 않는다"고 적어둔 것은 스테이징이
  앞설 때 이야기였는데, **운영이 앞서는 쪽이 더 나쁘다** — 스테이징이 운영을 대변하지
  못하게 된다. 배포는 한쪽만 끝내고 손을 떼지 않는다.
- **버전 스탬프는 바이트가 바뀔 때 같이 바뀌어야만 믿을 수 있다.** 소모임 캐시 수정은
  `app-core.js` 만 건드렸고 그 사이 다른 작업이 버전을 341 로 올려둔 상태였다. 그대로
  배포했으면 운영과 스테이징이 **둘 다 "v341" 이라고 말하면서 다른 코드를 서빙**하고,
  `app-core.js?v=341` 을 이미 캐시한 클라이언트는 옛 파일을 계속 썼을 것이다.
  파일 하나만 고쳤어도 버전은 올린다.
- **저장소가 다른 세션과 동시에 움직일 수 있다.** 내가 338→339 로 올린 것이 341 로
  바뀌어 있었고, 만들지 않은 커밋과 문서가 들어와 있었다. `git add -A` 로 남의 변경을
  쓸어 담지 않았는지 커밋 직후 `git show --stat` 으로 파일 목록을 확인한다.

## 2026-09-02 배포한 코드가 브라우저에 도착하지 않은 건

- **`cleanUrls: true` 는 헤더 규칙을 무력화한다.** `admin.html` 은 `/admin` 으로
  서빙되는데, 헤더 `source` 는 **요청 경로**에 맞춰지므로 `**/*.html` 이 걸리지 않는다.
  규칙에 안 걸린 경로는 Hosting 기본값 `max-age=3600` 이 붙어 **한 시간 동안 낡은
  파일이 돈다.** `/admin` `/changelog` `/privacy` `/terms` `/tokenomics`
  `/community-history` `/en/privacy` `/en/terms` 여덟 곳이 그랬다.
- **증상이 코드가 아니라 배송 문제일 수 있다.** "관제탑 열면 계속 에러나" 를 코드에서
  찾았고 실제로 고칠 것도 있었지만, 배포 후에도 브라우저는 **한 시간 전 파일**을 돌리고
  있었다. `window.reloadMemberList` 가 방금 배포한 함수인데 `undefined` 였다.
  고친 것이 화면에 없으면 **먼저 서버 파일과 브라우저 파일이 같은지 본다**:
  `fetch(url).then(r=>r.text())` 로 받은 내용에 새 코드가 있는데 `window.<새함수>` 가
  없으면 캐시다.
- **캐시 헤더는 로그가 아니라 `curl -I` 로 판정한다.** 2026-08-2x 의 "배포 성공은
  curl 로 판정한다" 와 같은 자리다. 배포 후:
  `curl -sI https://habitschool.web.app/admin | grep -i cache-control`
- **비어 있는 화면은 이유를 말해야 한다.** 회원 607명인데 "총 0명" 이라고 적혀 있었다.
  불러오는 중 · 실패 · 진짜 0건은 서로 다른 상태인데 한 문장을 공유하면, 평범한 콜드
  스타트가 고장으로 읽힌다. 목록·표·카드를 그리는 코드는 **loading / ready / error 를
  구분해서 말한다.**
- **타임아웃은 읽기 방식을 바꾸면 같이 다시 잰다.** `ADMIN_DASHBOARD_CORE_TIMEOUT_MS`
  9초는 Firestore 직접 읽기 시절 값인데, 목록을 콜러블로 옮긴 뒤에도 그대로였다.
  콜드 스타트가 9초를 넘기면 빈 배열이 렌더링됐다. **읽는 경로를 바꾸면 그 경로에
  걸린 제한시간도 근거를 다시 확인한다.**
- **`firebase deploy` 는 활성 별칭으로 간다.** 운영 배포하려다 `--project` 없이 실행해
  staging 으로 나갔다. 운영은 항상 `--project prod` 를 명시한다.

## 2026-09-02 20초의 원인은 조회가 아니라 연결이었다

- **한 탭이 느리면 그 탭을 보고, 여러 탭이 느리면 공통 경로를 본다.** "회원 목록이
  20초" 로 시작해 회원 목록 코드를 팠지만, 보상마켓 콘솔이 답을 갖고 있었다 —
  Firestore 읽기 **4개가 동시에** 타임아웃했다. 한 쿼리가 느린 게 아니라 **그 페이지의
  첫 Firestore 연결**이 모든 타임아웃 예산보다 비쌌다. 증상이 여러 화면에 퍼져 있으면
  화면이 아니라 그 아래 공통 계층을 의심한다.
- **`experimentalAutoDetectLongPolling` 은 공짜가 아니다.** 연결마다 "WebChannel
  스트리밍이 되는가"를 알아보는 왕복을 한 번 더 쓴다. 같은 브라우저 실측:
  auto-detect 555·568ms / force long-polling 44·57ms / 옵션 없음 305·366ms.
  **이 앱은 `onSnapshot` 이 하나도 없다** — 전부 한 번짜리 읽기라 그 답이 아무 이득도
  주지 않는다. 콘솔의 Listen 채널 404 도 이 프로브가 남기던 것이다.
- **전송 설정을 바꾸기 전에 `onSnapshot` 사용처를 센다.** 리스너가 생기면 롱폴링
  고정은 반대로 비싸진다. 이 전제를 테스트로 못박아 뒀다(tests/firestore-transport).
- **이전 결정을 뒤집을 때는 그때의 이유를 찾아 읽는다.** `git log -S <옵션이름>` 으로
  8d59e89 를 찾아보니 되돌린 이유가 **콘솔 노이즈**였고 성능이 아니었다. 그 노이즈를
  막는 가드는 같은 커밋에서 들어와 지금도 있다. 이유를 모르고 뒤집으면 남이 이미
  치른 값을 다시 치른다. 코드 주석에 그 이력을 남겨 뒀다.
- **화면이 기다리는 것과 화면이 쓰는 것을 맞춰 본다.** 회원 목록은 `daily_logs` 500건
  조회를 기다렸는데, 그건 **공복혈당·혈압 두 칸에만** 쓰였다. 나머지는 전부 콜러블
  payload 에 있었다. 늦게 와도 되는 것 때문에 다 기다리게 하지 않는다 — 먼저 그리고
  나중에 채운다.
- **관문을 낮출지는 표본으로 정한다.** 운동 영상 500개: 중앙값 17.14MB · 6초 ·
  23.1Mbps, 4-8MB 구간 47개(9.4%). 8MB 관문이 그 47개를 판단조차 안 하고 버리고
  있었다. **막는 관문과 고르는 관문을 구분한다** — 앞 관문은 "열어 볼 값이 있는가"만
  보고, 판단은 길이를 아는 뒤 관문에 넘긴다.

## 2026-09-02 내가 만든 병목 — 측정을 잘못 골랐다

- **한 건씩 잰 지연으로 동시성 문제를 판단하지 않는다.** `experimentalForceLongPolling`
  을 고른 근거는 `auto-detect 555ms 대 force 44ms` 였다. 그 숫자는 맞았지만 **순차
  조회 한 건**을 잰 값이라, 실제로 벌어진 일(유휴 롱폴링 채널 7개가 동시에 24~30초씩
  매달려 브라우저의 호스트당 연결 수를 다 먹는 것)을 보여주지 못했다. 그 뒤에 나가는
  조회가 전부 줄을 섰고, 보상마켓 읽기 4개가 7초 타임아웃에 걸렸다.
  **동시에 열리는 것의 개수와 지속 시간**을 봐야 했다.
- **DevTools Network 를 Time 과 Size 두 번 정렬해 본다.** 콘솔은 "timed out" 이라고만
  말하지 무엇이 자리를 차지했는지 말하지 않는다. Time 정렬이 25초짜리 0.1kB 채널
  7개를 보여줬고, Size 정렬이 `getAdminMembers 432kB × 2` 를 보여줬다. 각각 다른
  버그였고 둘 다 콘솔에는 흔적이 없었다.
- **내 환경에서 빠르면 해결됐다고 보고하지 않는다.** 같은 PC에서도 자동화 탭과 사용자
  탭이 달랐다. "빠르게 나온다"를 네 번 보고했고 네 번 다 틀렸다. 두 번째로 어긋났을
  때 코드가 아니라 **환경 차이부터 좁혔어야** 했다. 좁히는 가장 빠른 수단은 사용자
  화면의 Network 기록이었고, 그건 내가 먼저 요청했어야 할 것이다.
- **실패가 화면에 안 보이면 진단이 시작되지 않는다.** `loadTab` 이 오류를 `console.error`
  로만 남겨서, 사용자는 "로딩 중..." 만 30초 보고 나는 원인을 못 봤다. 화면에 이유를
  적게 만든 그 배포 직후 `기록 데이터를(를) 9초 안에 받지 못했습니다` 한 줄이 나왔고
  거기서 진짜 원인이 풀렸다. **관측 가능성을 먼저 고치는 것이 지름길이다.**
- **되돌린 결정을 다시 뒤집을 때는 근거의 종류가 달라야 한다.** 5월(8d59e89)의 선택을
  뒤집으면서 나는 그때보다 나은 근거를 갖고 있다고 믿었지만, 실은 더 좁은 근거였다.
  이력을 읽는 것(`git log -S`)만으로는 부족하다 — **그 결정이 답하던 질문에 내 측정이
  답하는지**를 확인해야 한다.

## 2026-09-04 상태를 말하는 화면은 그 상태를 벗어날 길도 갖고 있어야 한다

이번 주에 같은 모양의 버그를 세 번 고쳤다. 증상은 다 달랐지만 구조가 하나였다.

| 화면이 말한 것 | 실제로 빠져 있던 것 |
|---|---|
| `총 0명` | 불러오는 중과 아무도 없음이 같은 문장이었다 |
| `로딩 중...` 30초 | 실패했는데 `console.error` 로만 남기고 화면은 그대로였다 |
| `정산 확인 중…` | 기다린다고 적어 두고, 그 확인을 **아무도 요청하지 않았다** |

- **문구를 쓸 때 "이게 사라지려면 무엇이 일어나야 하고, 그걸 누가 하는가"를 같이 적는다.**
  셋 다 화면은 상태를 정확히 표시했다. 거짓말이 아니라 **막다른 길**이었다.
  답이 "다른 어딘가에서 언젠가"면 그건 없는 것이다 — 챌린지 정산은 실제로
  `js/auth.js` 의 로그인 10초 뒤 타이머가 유일한 탈출구였고, 그래서 회원들은
  앱을 껐다 켜야 했다.
- **"한 번만 참인 조건"으로 요청을 걸지 않는다.** 챌린지 재계산은
  `if (!projection.changed) return;` 안에서만 나갔다. 로컬 추정이 **바뀌는 순간**에만
  참이므로 기회는 평생 한 번이고, 그 한 번을 놓치면(모듈 로드 전, 콜러블 실패,
  지연 중 탭 이동) 영원히 다시 시도하지 않는다. **변화가 아니라 불일치를 조건으로
  삼는다** — "추정과 저장된 상태가 어긋나 있으면 요청한다" 로 바꾸면 스스로 낫는다.
  중복은 이미 있던 60초 dedupe 가 막고, 서버가 상태를 넘기면 조건이 거짓이 되어
  저절로 멈춘다.
- **서버만 고치고 화면을 두고 오지 않는다.** `settleExpiredChallenges` 머리말에 이미
  같은 실패가 적혀 있었는데(2026-08), 다른 호출 지점에서 똑같이 반복했다.
  **한 곳에서 이런 실수를 고치면 같은 패턴을 다른 호출부에서도 찾아본다.**
  이번 주 `getData()` 건도 회원 목록에서 한 번 고치고 세 군데를 놓쳐 세 번에 나눠
  고쳤다.
- **관측 가능성을 먼저 고치면 나머지가 빨라진다.** 탭 실패를 화면에 적게 만든 배포
  직후 `기록 데이터를(를) 9초 안에 받지 못했습니다` 한 줄이 나왔고, 이틀 걸리던
  진단이 거기서 끝났다.

## 2026-09-07 새는 곳을 통 바닥에서 찾고 있던 건

- **"활성 8%"는 잔존 문제로 읽히지만, 어디서 새는지 말해 주지 않는다.** 8/31 외부
  홍보 전략은 "579명을 모아 48명이 남았다 … 새는 통을 먼저 막지 않으면 붓는 만큼
  손해"라고 적고 잔존 개선을 다음 순서로 뒀다. 실제로 재 보니 **608명 중 431명
  (70.9%)이 단 하루도 기록하지 않았다.** 통은 바닥이 아니라 **입구**에서 샜다.
- **분모를 갈라 보기 전까지는 유지력을 판단할 수 없다.** 한 번이라도 기록한 사람
  중에서는 29%(51/177)가, 3일까지 간 사람 중에서는 **62%(51/82)가 지금도 활성**이다.
  전체 대비 8%라는 한 숫자가 "제품이 사람을 못 잡는다"로 읽혔지만, 잡는 힘은
  멀쩡했고 시작을 안 할 뿐이었다. **비율을 볼 때 분모가 '가입자'인지 '써 본
  사람'인지를 먼저 적는다.**
- **기계가 안 도는 것과 돌릴 사람이 없는 것은 다르다.** 초대·공유 기계는 4~9월에
  걸쳐 완성돼 있는데 초대 유입이 2.8%였다. 원인을 기계에서 찾고 있었지만,
  공유 유도 시트가 열릴 수 있는(65P 달성) 회원이 최근 30일에 **6명**이었다.
  분모가 없는 전환율을 손보는 것은 일이 아니다. 2026-08-21 의 "순위를 만들 사람이
  없으면 가중치 논의는 의미가 없다"와 같은 실수를 다른 화면에서 반복할 뻔했다.
- **건수만 보면 편중이 안 보인다.** 9월 리액션 325개가 **4명**에게서 나왔다.
  8월에도 494개 중 356개가 한 사람이었다. **커뮤니티 지표는 건수와 고유 인원을
  항상 같이 적는다** — 건수만 보면 활발해 보인다.
- **교차 검증은 창을 맞춰서 본다.** 이 측정의 활성 51명과 관제탑의 29명은
  어긋난 것이 아니라 최근 30일 vs 달력 월이었다. 창이 같은 항목(9월 댓글 2개)은
  정확히 일치했다. **창이 같은 항목을 하나 이상 넣어 두면 스크립트가 틀렸는지
  창이 다른지를 즉시 가를 수 있다.**

## 2026-09-07 원인을 고치고 그 원인 때문에 좁혀둔 조건은 안 넓힌 건

- **회피책은 원인이 사라져도 저절로 풀리지 않는다.** 삼성 인터넷 일반 탭은
  2026-04-27 저녁부터 팝업 로그인을 썼다. 그날 아침 redirect 로 바꿨다가
  "구글에서 돌아오면 로그인 화면으로 튕긴다"는 회귀 때문에 `&& isStandalone` 을
  붙여 되돌린 것이다. 그 튕김의 진짜 원인(리디렉트 중간 상태를 서드파티
  authDomain 저장소에 맡기던 것)은 **2026-08-12 에 고쳤다**(94aebe9). 그런데
  그것 때문에 좁혀 둔 조건은 아무도 다시 넓히지 않았고, 그 뒤로도 설치하지 않은
  삼성 인터넷 사용자는 계속 팝업을 타고 구글 계정 화면이 Gmail 앱으로 넘어가
  메일함에서 로그인이 끝났다. **원인을 고치는 커밋에는 "이 때문에 우회했던
  곳"을 같이 찾아 되돌리는 일이 포함된다.**
- **되돌린 이력이 있는 조건은 그냥 다시 뒤집지 않는다.** 이 조건은 하루 만에
  popup → redirect → popup 으로 왕복했다. 양쪽 다 실제 실패가 있었기 때문이다.
  세 번째로 건드리기 전에 *되돌린 쪽의 이유가 아직 살아 있는지*부터 확인해야
  한다. 이번엔 그 이유가 8월에 제거돼 있었고, 전제(`/__/auth/handler` 가 우리
  도메인에서 462바이트 진짜 핸들러로 서빙되는지)를 curl 로 직접 찍어 확인한 뒤에
  넓혔다.
- **가입 화면 앞에서 잃은 사람은 어떤 지표에도 안 잡힌다.** 로그인을 못 끝내면
  `users` 문서가 아예 안 생긴다. 그래서 이 손실은 회원 608명 기준의 퍼널
  어디에도 나타나지 않고, 코호트·잔존·활성 어느 표에서도 0으로조차 보이지
  않는다. **분모 밖에서 새는 것은 분모 안의 숫자를 아무리 봐도 안 보인다** —
  브라우저별로 나눠 보는 계측(GA 의 브라우저 차원 + `auth_result`)이 있어야 한다.
- **테스트가 회피책을 계약으로 굳혀 놓을 수 있다.** `keeps popup for Samsung
  Internet in normal browser tabs` 는 임시 회피를 정상 동작으로 못 박고 있었다.
  다만 같은 파일의 다른 테스트가 "여기가 바뀌면 authDomain 규칙도 같이 봐야
  한다"는 결합 가드를 남겨 둔 덕에, 조건을 넓힐 때 무엇을 확인해야 하는지
  바로 알 수 있었다. **회피책을 테스트로 굳힐 때는 왜 그렇게 두는지와 언제
  풀어야 하는지를 같이 적는다.**

## 2026-09-07 지표 이름만 보고 무엇을 재는지 확인하지 않은 건

- **이벤트 이름이 뜻을 보장하지 않는다.** 브라우저별 로그인 성공률을 보려고
  `auth_result` 를 쓰겠다고 계획서에 적고 사용자에게도 그렇게 안내했는데,
  이 이벤트의 유일한 발생 지점은 `js/guest-demo.js:1045` 였다. **게스트 데모를
  거친 로그인만** 잡히고, 로그인 화면에서 바로 구글 버튼을 누르는 다수 경로는
  이벤트가 없다. 7~9월 `auth_result` 34건 vs 같은 기간 신규 가입 197명이
  그 증거였는데, 숫자를 보기 전에는 눈치채지 못했다.
  **계측을 쓰기 전에 emit 지점을 grep 한다.** 이름과 스키마만 보면 안 된다.
- **가설을 세운 사람이 그 가설에 맞는 지표를 고르면 확증만 남는다.** "삼성
  인터넷 세션은 많은데 auth_result 가 0에 가까울 것" 이라고 예고까지 했다.
  실제로는 삼성 인터넷 222명이 675건을 기록하고 있었다 — 막혀 있지 않았다.
  실기기에서 Gmail 로 새는 것을 재현한 것은 사실이지만, **재현했다는 것과
  그것이 큰 손실이라는 것은 다른 주장이고 근거도 달라야 한다.**
- **로그인 이후에만 가능한 이벤트가 로그인 여부를 더 정확히 말해 준다.**
  `record_saved` 는 로그인 없이는 발생할 수 없으므로, 브라우저별 세션 대비
  `record_saved` 가 로그인 도달률의 하한이 된다. 직접 재는 지표가 없을 때는
  **그 뒤에만 일어날 수 있는 일**을 대신 센다.
- **분모 밖의 손실은 한 겹이 아닐 수 있다.** 회원 608명 퍼널에서 431명이
  기록 0이었고, 그 앞에 로그인 못 한 사람이 또 있고, 그 앞에 **Android
  Webview 282명 · 웨일 135명이 아무 이벤트도 남기지 않고** 사라져 있었다.
  가입 기준으로만 보면 이 417명은 존재 자체가 안 보인다.

## 2026-09-08 잠근 이유를 잠근 자리에서 말하지 않은 건

- **`disabled` 로 잠그면 왜 잠겼는지 말할 기회까지 잠긴다.** 동의 게이트가
  로그인 버튼에 `disabled` 를 걸고 안내는 `title` 에만 뒀다. disabled 인 버튼은
  **클릭 이벤트가 발생하지 않고**, 모바일에는 툴팁이 없다. 회색 버튼을 누른
  사람에게는 아무 일도 일어나지 않는다. 만든 사람조차 이것을 버그로 신고했다.
  **막을 때는 `aria-disabled` 로 모양만 잠그고 클릭은 받아서 이유를 말한다.**
- **막힌 사람은 그 앞 단계 지표에 안 잡힌다.** `auth_start` 는 로그인 버튼이
  실제로 동작했을 때 나간다. 동의에서 막힌 사람은 로그인을 시작조차 못 하므로
  auth_start 에도, auth_result 에도 없다. **관문을 만들 때 그 관문에서 돌아선
  사람을 세는 이벤트를 같이 만든다.** 안 그러면 그 이탈은 존재하지 않는 것이 된다.
- **`requestAnimationFrame` 은 탭이 숨겨지면 실행되지 않는다.** 애니메이션을
  다시 돌리려고 클래스를 뺐다 넣는 사이를 rAF 로 띄웠더니, 숨은 탭에서 빨간
  표시가 영영 안 붙었다(브라우저 확인에서 잡혔다). 화면에 반드시 나타나야 하는
  변화는 rAF 에 걸지 않는다 — 강제 리플로우(`void el.offsetWidth`)로 동기 처리한다.
- **가드 테스트는 의도를 잠그되 방식은 잠그지 않아야 한다.** `signup-consent`
  테스트가 `loginBtn.disabled = !ready` 라는 **구현**을 문자열로 고정하고 있어서,
  더 나은 방식으로 바꿀 때 같이 고쳐야 했다. 이번엔 단언을 "클릭 핸들러가
  필수 동의 없이는 로그인을 시작하지 않는다" 라는 **보장**으로 옮겼다.

## 2026-09-08 브라우저를 사람으로 착각한 건

- **기기에 남긴 표시는 "누가" 를 모른다.** 동의 화면을 "이미 동의한 브라우저"
  에게 감추는 기능이 있었다. 그 표시(`habitschool-consent-accepted`)는
  localStorage 에 **브라우저 단위**로 남고 로그아웃해도 지워지지 않았다.
  그래서 로그아웃한 기기에서 다른 사람이 가입하면 동의 화면을 아예 못 봤고,
  **앞사람의 선택으로 그 사람의 동의 기록이 만들어졌다.** 화면이 비는 것보다
  기록이 거짓인 쪽이 훨씬 큰 문제다 — 개인정보 보호법 제22조의 동의는 본인이
  한 것이어야 한다.
- **"다시 묻지 않기" 를 만들 때는 무엇을 근거로 같은 사람이라고 보는지 적는다.**
  로그인 화면은 정의상 아직 누구인지 모르는 화면이다. 거기서 사람을 특정하는
  근거는 존재할 수 없으므로, 그 화면의 "다시 묻지 않기" 는 성립하지 않는다.
- **미리 체크해 두는 것도 같은 잘못이다.** 상자를 보여 주더라도 필수 항목이
  미리 체크돼 있으면 본인이 한 동의가 아니다. 상자를 감추는 것과 미리 체크하는
  것은 한 몸이라 같이 걷어내야 한다.
- **삭제한 기능에 테스트가 있으면 계약을 다시 쓴다.** `someone who already
  agreed is not asked again` 이라는 describe 블록 전체가 이 동작을 지키고
  있었다. 지우고 끝내지 않고 **새 보장**(로그인 화면은 항상 묻는다 / 로그아웃이
  표시를 지운다 / 기록은 남긴다)으로 바꿔 적었다. 그래야 다음 사람이 "예전엔
  안 묻게 돼 있었는데" 하고 되돌리지 않는다.

## 2026-09-08 증상을 막고 순서는 그대로 뒀던 건

- **잘못된 자리에서 받은 것을 "잘 받게" 고치면 자리는 그대로 남는다.** 로그아웃한
  기기에서 다음 사람이 앞사람 동의를 물려받던 문제를, 처음에는 "로그아웃 때
  표시를 지우고 미리 체크하지 않는다"로 고쳤다(v369). 증상은 사라졌지만
  **동의가 여전히 브라우저에 붙어 있어서**, 같은 사람이 로그아웃했다 들어와도
  다시 물어야 했다. 사용자가 "그건 말이 안 되지" 라고 해서 순서를 봤다.
  **고치기 전에 "이 값은 무엇에 붙어야 하는가"를 먼저 묻는다** — 계정인가
  브라우저인가 기기인가. 답이 정해지면 어디서 받을지는 따라온다.
- **로그인 화면은 누구인지 모르는 화면이다.** 거기서 사람에 대해 아무것도
  기억할 수 없다. 동의처럼 사람에 붙어야 하는 것은 **로그인 뒤**로 가야 한다.
  옮기고 나니 리디렉트 왕복 중 체크박스를 잃는 문제(스냅샷 저장·복원 코드
  전부)가 통째로 사라졌다 — **자리를 바로잡으면 그 자리 때문에 있던 보조
  장치들도 같이 없어진다.**
- **이미 절반이 있었다.** 기존 회원 재동의용 `needsConsentRefresh` + 모달이
  정확히 "로그인했는데 동의가 없는 사람" 을 처리하는 장치였는데,
  `!isNewUser` 로 신규를 빼 놓아 두 경로가 갈라져 있었다. **새로 만들기 전에
  비슷한 일을 하는 코드가 이미 있는지 본다** — 조건 하나를 지우는 것이
  새 화면을 만드는 것보다 낫다.
- **삭제한 자리마다 테스트가 있었다.** 4개 파일 22건이 깨졌고, 전부 "동의를
  로그인 화면에서 받는다"는 **위치**를 고정하고 있었다. 지우지 않고 같은
  보장을 새 위치로 옮겨 적었다. `consent-survives-redirect` 는 전제가
  사라져 `consent-belongs-to-the-account` 로 이름까지 바꿨다 —
  **파일 이름이 낡은 설계를 가리키고 있으면 다음 사람이 그 설계를 믿는다.**

## 2026-09-08 두 창이 동시에 뜬 건, 그리고 줄바꿈이 테스트를 깨뜨린 건

- **로그인 직후에 뜨는 창이 둘이면 순서를 정해 줘야 한다.** 동의 창과 온보딩
  (습관 고르기)이 각자 `setTimeout` 으로 떠서, 동의 창이 잠깐 보였다가 온보딩이
  덮었다. 신규 회원은 동의 창을 보지도 못하고 지나쳤다.
  **막는 쪽 한 곳만 고치면 다른 경로로 또 덮인다** — 호출부가 아니라
  `checkOnboarding` 함수 자체가 관문이 열려 있으면 물러나게 했고,
  동의를 마치는 쪽이 다시 부르게 했다. 미루기만 하고 다시 부르지 않으면
  신규 회원이 습관 고르는 화면을 영영 못 본다.
- **없는 CSS 클래스를 쓰면 숨긴 줄 알았던 것이 화면에 찍힌다.** 스크린리더용
  제목에 `sr-only-title` 을 썼는데 그런 클래스가 없어서 「동의」가 상자 위에
  그대로 나왔다. 실기기 스크린샷으로 잡혔다. **클래스를 쓸 때는 그 클래스가
  실제 CSS 에 있는지 확인한다** — 이 저장소의 것은 `.sr-only`(styles-dashboard.css)다.
- **파일을 쓰는 도구가 줄바꿈을 바꿀 수 있다.** Windows 에서 python 의
  `open(path, 'w')` 는 `\n` 을 `\r\n` 으로 바꿔 저장한다. 그렇게 16개 파일이
  LF → CRLF 가 됐고, **소스를 문자열로 읽어 여러 줄을 비교하는 테스트 6건이
  한꺼번에 깨졌다.** 화면도 번들도 멀쩡해서 원인을 코드에서 찾느라 한참 걸렸다.
- **줄바꿈 때문에 깨진 단언을 "계약이 바뀐 것"으로 착각하면 진짜 계약이 잘못
  고쳐진다.** 이번에 `AUTH.split('\n}')` 류 단언이 여럿 빗나갔는데, 그중 일부는
  실제 코드 변경 때문이고 일부는 CRLF 때문이었다. 되돌려 놓고 다시 돌려 보기
  전까지는 둘을 구분할 수 없었다. **여러 단언이 동시에 깨지면 코드보다 먼저
  파일 자체(인코딩·줄바꿈)를 의심한다.**
- 재발 방지로 `tests/source-line-endings.test.js` 를 뒀다. 주요 소스에 CRLF 가
  섞이면 바로 잡힌다.

## 2026-09-08 잠근 함수가 풀지 않고 남에게 맡기고 있던 건

- **상태를 세운 함수가 거두지 않으면, 거두던 사람이 사라질 때 같이 사라진다.**
  `setGoogleLoginPendingUi(btn, true)` 는 `disabled` 를 걸었는데 `false` 분기는
  `aria-busy` 와 문구만 되돌렸다. 실제로 `disabled` 를 푸는 것은 그 아래에서
  부르던 `syncSignupConsentState()` 였고, 그건 **동의 상태를 보려고 있던 함수지
  대기 상태를 거두려고 있던 함수가 아니었다.** 동의를 로그인 뒤로 옮기며 그
  함수를 지우자 푸는 곳이 없어져, 로그아웃한 뒤 새로고침 전까지 시작 버튼이
  잠겼다. **세우는 쪽과 거두는 쪽을 한 함수 안에서 짝지어 둔다.**
- **우연히 그 일을 해 주던 코드는 지울 때 티가 안 난다.** 지운 함수의 이름은
  동의였고 사라진 효과는 버튼 잠금 해제였다. 이름만 보고 지우면 놓친다.
  **삭제 전에 "이 함수가 하는 일" 이 아니라 "이 호출 지점에서 기대되는 효과" 를
  적어 본다.**
- **같은 증상을 두 경로에서 보고받으면 합류 지점에서 막는다.** 로그아웃 버튼과
  동의 창의 '그만두기' 가 각각 보고됐고 둘 다 `logoutAndReset` 으로 모인다.
  비로그인 셸이 그려질 때도 거두지만 그건 `onAuthStateChanged` 가 돌아야 하는
  일이라, 로그아웃 자체가 먼저 거두게 했다.

## 2026-09-08 재현했다고 원인을 안다고 착각한 건

- **증상을 재현한 것과 원인을 찾은 것은 다른 일이다.** 삼성 인터넷에서 구글
  로그인이 Gmail 로 새는 것을 실기기로 재현했고, 마침 팝업/리다이렉트 조건이
  4월에 좁혀진 채 방치돼 있는 것을 발견해 **그것을 원인으로 단정했다.** 코드를
  고치고, "8월 이후 계속 새고 있었다"고 적고, 4개월치 신규 회원을 먹었을 수
  있다고까지 말했다. 리다이렉트로 바꾼 뒤 한 번 성공한 것을 확인의 근거로 삼았다.
- **진짜 원인은 기기 설정이었다.** 다음 날 같은 증상이 기본형·영문판 모두에서
  다시 났고, 그때 **탭 개수가 늘지 않았다** — 새 탭(팝업)이 아니라 같은 탭
  이동(리다이렉트)이었다. 코드는 의도대로 돌고 있었는데도 안드로이드가
  `accounts.google.com` 이동을 Gmail 앱에 넘긴 것이다. Gmail 의 「지원되는 링크
  열기」를 끄자 멈췄다.
- **한 번 성공한 것은 확인이 아니다.** 간헐적 증상에서 "고치고 나니 됐다"는
  증거로 너무 약하다. 두 번째 보고가 없었다면 틀린 인과를 그대로 문서에
  남겼을 것이다. **간헐적인 것은 재현 조건을 먼저 찾고, 그 조건에서 전후를 본다.**
- **구분 실험을 먼저 설계했어야 했다.** 팝업이면 새 탭이 열리고 리다이렉트면
  같은 탭이 바뀐다 — **1초짜리 관찰로 갈리는 문제**를 코드를 고치고 배포한 뒤
  이틀 만에야 물어봤다. 가설이 두 개면 그 둘을 가르는 관찰부터 찾는다.
- 리다이렉트 전환 자체는 되돌리지 않았다. 2026-08-12 의 authDomain 수정 혜택을
  일반 탭도 받게 되는 별개의 근거가 있다. **다만 그 변경을 이 증상의 해결책으로
  소개한 것은 취소했다** — 근거 없는 인과를 문서에 남기면 다음 사람이 그것을
  사실로 읽는다.

## 2026-09-08 화면에 로그인 버튼이 없는데 로그인 방식을 고치고 있던 건

- **웨일 회원 135명이 기록 0건이던 진짜 이유는 로그인 버튼이 아예 안 보인 것이었다.**
  `js/auth.js` 의 `isWebView()` 인앱 브라우저 목록에 `/Whale\//i` 가 들어 있었고,
  주석도 "네이버 웨일 또는 WebView" 로 둘을 뭉뚱그려 놨다. 이 함수가 참이면
  `loginBtn.style.display = 'none'` 이다 — **로그인할 방법이 없다.**
  네이버 웨일은 독립 브라우저이고 구글 로그인이 정상 동작한다.
- **나는 그 앞 단계를 확인하지 않고 로그인 '방식'을 고쳤다.** 9/7 에 웨일을
  리다이렉트로 돌리면서 "삼성과 같은 모양" 이라고 적었는데, 삼성과 달리 웨일은
  로그인을 **시작조차 못 하고 있었다.** 가입 클릭 19건은 게스트 데모의 버튼이었고,
  그 뒤에 로그인 화면이 없었다. 재현 없이 정황만으로 처방한 결과다.
  근거가 사라졌으므로 그 리다이렉트는 되돌렸다.
- **"전환이 0이다" 를 보면 전환 방식이 아니라 그 앞 관문부터 본다.** 19 → 0 은
  "누르는데 안 된다" 보다 "누를 것이 없다" 에 가까운 모양이었다. 화면을 한 번
  열어 봤으면 5초에 끝났을 일을 데이터로 추측했다. **사용자가 보는 화면을 먼저
  본다.**
- **같은 목록이 세 벌로 흩어져 있다.** 인앱 브라우저 판정이 `js/auth.js`
  (로그인 차단), `js/pwa-install.js`(설치 안내), `js/webview-detect.js`(외부
  브라우저 이동) 에 각각 있고 내용이 서로 다르다. 웨일은 앞의 둘에만 있었다.
  **판정이 여러 벌이면 언젠가 갈라지고, 갈라진 쪽이 사용자를 막는다.**

## 2026-09-08 목록에서 이름만 빼고 고쳤다고 했다

- **웨일을 `isWebView()` 목록에서 뺐는데 화면은 그대로였다.** 같은 판정이
  `js/auth.js`(로그인 차단)·`js/pwa-install.js`(설치 안내)·`js/webview-detect.js`
  (자동 이동) 세 곳에 복사돼 있었고 내용이 서로 달랐다. 게다가 웨일은 크로미움
  기반이라 UA 에 `; wv)` 같은 WebView 토큰을 함께 싣는 형태가 있어서, 이름을
  빼도 남은 패턴이 계속 잡았다.
- **한 곳을 고치고 "고쳤다"고 보고했다.** 배포까지 하고 사용자가 같은 화면을
  다시 보고서야 알았다. 판정이 여러 벌이라는 건 전날 lessons 에 내가 직접
  적어 놓고도, 정작 고칠 때는 한 벌만 봤다.
- **블록리스트로 "정상"을 정의하려 하지 말 것.** 목록은 계속 늘고, 언젠가
  멀쩡한 브라우저를 잡는다. 막히는 것이 로그인이면 그 사용자는 되돌아올
  방법이 없다. 그래서 물음을 뒤집었다 — "인앱처럼 보이나"가 아니라 "알려진
  독립 브라우저인가"를 먼저 묻고, 맞으면 어떤 규칙도 그 판단을 못 뒤집는다.
- **비대칭을 기본값으로 옮긴다.** 잘못 막으면 영영 못 들어오고, 잘못 통과시키면
  팝업이 한 번 실패할 뿐이다. 그래서 판정 모듈을 못 읽으면 `false`(통과)다.
  **되돌릴 수 없는 쪽으로 기울어진 기본값을 남겨 두지 않는다.**
- **사람을 막는 분기에는 계측을 붙인다.** 이 분기는 로그인을 통째로 없애는데
  이벤트가 하나도 없었다. 그래서 GA 에는 "가입 클릭은 있고 기록은 0" 으로만
  보였고, 나는 그 모양을 로그인 '방식' 문제로 두 번 오독했다.
  `auth_browser_blocked` 를 붙였다 — 다음엔 세어 보고 말한다.

## 2026-09-10 배포는 커밋이 아니라 작업 디렉터리를 올린다

- **운영 배포 직전에 작업 트리에서 남의 작업을 발견했다.** 같은 저장소에서 다른
  세션이 Health Connect 브리지를 작업 중이었고, 기능 플래그가 켜지고 버튼이
  추가되고 버전이 v381 로 올라간 상태였다. 전부 커밋 전이었다.
- **`firebase.json` 의 hosting 은 `"public": "."` 이다.** 그냥 `firebase deploy`
  했으면 검증한 적 없는 미완성 기능이 운영에 그대로 나갔다. 배포되는 것은 내가
  커밋한 것이 아니라 **그 순간 디스크에 있는 것 전부**다.
- **그래서 배포 전에 `git status` 를 본다.** 내가 만들지 않은 변경이 있으면
  거기서 멈춘다. 이번에는 `git worktree add --detach <임시경로> HEAD` 로 커밋된
  상태만 꺼내 그 디렉터리에서 배포했다. 확인은 배포 후 URL 로 다시 했다 —
  운영 첫 화면에 그 버튼이 없는지(`grep -c` 가 0), 버전이 v380 인지.
- **"내가 검증한 것"과 "배포되는 것"이 같다고 가정하지 않는다.** 스테이징에서
  확인한 것은 내 커밋이었지만, 배포 명령이 집어 올리는 것은 디렉터리였다.
  둘 사이가 벌어질 수 있다는 것을 배포 순간에야 알았다.
- 남의 작업 파일은 되도록 건드리지 않는다. 이번엔 en 동기화 테스트를 고치려고
  `en/index.html` 을 재생성했는데, 그게 상대 세션의 버전 번호를 함께 반영했다.
  결과적으로 어긋난 것을 맞춘 것이지만, **내 작업이 아닌 파일을 만졌다는 사실은
  말해야 한다.**

## 257. 침묵하는 원격 호출에는 마감선을 걸어 둔다 (2026-09-14)

운동 AI 분석이 '분석 중...' 에서 멈췄다는 제보. 함수 로그에는 요청 도착만 있고
59초 뒤 흔적이 끝날 뿐, 어디서 늦었는지 한 글자도 없었다. 추측으로 고칠 뻔했다.

**로그를 먼저 심었다.** 이미지 내려받기 0.7초, 모델 응답 3.6초 — 멀쩡했다. 즉
그때 60초를 먹은 곳은 모델 호출 쪽이고, 거기엔 마감선이 없어 함수 타임아웃까지
아무 말도 못 했다. 원인은 끝내 확인하지 못했다(재현되지 않음).

규칙:
- 함수 안의 모든 외부 호출(fetch, 모델, 체인)에 **함수 타임아웃보다 짧은 마감선**을
  건다. 우리가 먼저 끊어야 왜 실패했는지 말할 수 있다.
- 마감선은 `deadline-exceeded` 같은 **화면이 보여줄 수 있는 코드**로 바꾼다.
...
`node --input-type=module <<'EOF'` 로 코드를 쓰면 `\s` `\n` 같은 이스케이프가
한 겹 벗겨져 파일에 들어간다. 이번에 두 번 당했다.

- `/```(?:json)?\s*([\s\S]*?)```/` → `/```(?:json)?s*([sS]*?)```/`
  (JSON 응답에 코드펜스가 없어 드러나지 않던 잠복 버그)
- 테스트 파일의 `.join('\n')` 이 진짜 줄바꿈이 되어 문법 오류
...
## 260. 스키마 번호를 올리기 전에 그 번호를 누가 찍는지 본다 (2026-09-18)

갤러리 게시물에 필드를 하나 늘리면서 클라이언트의
`GALLERY_PERSISTED_POST_SCHEMA_VERSION` 을 2 → 3 으로 올렸다. "기기에 남은 낡은
캐시를 버리게 하는" 흔한 수법이라고 생각했다.

그런데 그 번호는 클라이언트가 스스로 찍는 번호가 아니었다. 서버가
`functions/gallery-posts.js` 에서 게시물마다 `schemaVersion: GALLERY_POST_SCHEMA_VERSION`(2)
을 찍고, 클라이언트는 **읽은 게시물의 번호가 자기 상수와 같은지** 본다.

    Number(item.data.schemaVersion) !== GALLERY_PERSISTED_POST_SCHEMA_VERSION → 전부 버림

즉 한쪽만 3 으로 올리면 낡은 캐시만 버려지는 게 아니라 **모든 게시물이 걸러진다.**
새로 받아 저장해도 저장된 번호는 여전히 2 라 다음 읽기에서 또 버려진다 — 영구
캐시가 통째로, 조용히 죽는다. 화면은 멀쩡하고 매번 처음부터 받을 뿐이라 증상이
느림 하나뿐이다. 배포 전에 다른 이유로 그 코드를 다시 보다가 잡았다.

**규칙: 버전 상수를 올리기 전에 `grep` 으로 그 값을 쓰는 곳을 전부 본다.**

- 내가 찍고 내가 읽는 번호인가 → 혼자 올려도 된다
- 남이 찍고 내가 읽는 번호인가 → **양쪽을 같이 올리거나, 올리지 않는다**

그리고 짝이라면 짝이라는 사실을 시험으로 묶어 둔다. 양쪽에서 숫자를 읽어
같은지 보는 시험 하나면 된다(`tests/gallery-sleep-analysis.test.js`).

캐시를 무르게 하는 것이 목적이었다면 애초에 TTL 이 24 시간이었다. 하루만
기다리면 되는 일이었다.

---

## 261. 끝나지 않는 쓰기는 실패보다 조용하다 (2026-09-18)

"동의 화면이 왜 계속 뜨지?" 를 받고 서버를 읽었다. 그 계정에는 동의 기록이
없었다. 나는 "오늘 처음 저장됐다" 고 보고했다 — **타임스탬프를 잘못 읽은
것이다.** 동의를 누를 때마다 네 항목의 `at` 이 전부 지금 시각으로 덮어써지므로,
그 값은 "처음" 인지 "또" 인지 아무것도 말해 주지 않는다.

지적을 받고 다시 봤다. "매일 들어가는 계정인데 한 번도 안 했다니 말이 안 된다."
맞는 말이었다. 지우는 경로를 앱·서버·챗봇에서 전부 뒤졌지만 users 문서에
merge 없는 쓰기는 한 곳도 없었다. 지워진 것이 아니라 **닿은 적이 없었다.**

Firestore 쓰기는 **서버가 받았을 때** 약속이 풀린다. 연결이 끊긴 동안에는
로컬에만 적고 약속을 붙들고 있는다. 그래서 `await setDoc(...)` 은

- 거부되지 않는다 (catch 가 잡을 것이 없다)
- 끝나지도 않는다 (finally 도 오지 않는다)
- 로컬 읽기에는 성공한 것처럼 보인다

버튼은 잠긴 채, 토스트도 없이, 창만 떠 있었다. 사람은 새로고침으로 빠져나가고
기록은 남지 않는다. 다음 로그인에 같은 창이 또 뜬다.

**규칙 1: 사람이 기다리는 쓰기에는 시한을 건다.** `withAsyncTimeout` 으로
감싸고, 넘으면 저장된 척하지 않는다 — 버튼을 풀고, 창을 닫지 말고, 왜
안 됐는지 말한다.

**규칙 2: 읽기도 같다.** 같은 날 `getDocs` 가 빈 메모리 캐시를 오류 없이
빈 결과로 돌려주어 "이번 주 운동 0분" 을 만들었다. `getDoc` 은 오프라인이면
던지지만 `getDocs` 는 던지지 않는다. **모르는 것과 없는 것을 코드가 구분해야
한다** — `snapshot.metadata.fromCache` 가 그 답이다.

**규칙 3: 타임스탬프로 '처음' 을 주장하지 않는다.** 덮어쓰는 필드인지 먼저
본다. 데이터가 말하지 않는 것을 말했다가 사용자에게 정정당했다.

---

## 262. 서버에 없는 상태는 다시 그리기 한 번에 사라진다 (2026-09-21)

사흘 동안 같은 자리에서 세 번 걸렸다.

- 9/19 "운동 시간 누르면 AI분석 실패" → 저장이 분석을 안 기다렸다
- 9/20 "영상 올리는 사이에 저장하면 실패 · 식단 4장 중 3번째 실패"
  → **이미 시작된 분석**만 기다렸다. 업로드 중이면 기다릴 것이 없다.
  식단·수면은 대기 목록에 등록조차 안 돼 있었다
- 9/21 "분석 결과가 잠깐 나왔다가 새로고침처럼 깜빡이고 사라짐"
  → 일일 기록을 다시 읽으면 clearInputs 가 운동 블록을 다시 그린다

셋 다 원인이 하나다. **운동 분석 결과는 화면(data-ai-analysis)에만 있다.**
식단·수면은 persistAnalyzedDietPhotoAndResult 로 그 자리에서 서버에 들어가는데,
운동은 저장 버튼이 확정한다. 화면에만 있는 값은 화면을 다시 그리는 모든 경로가
지우개다 — 저장, 날짜 전환, 포커스 복귀, 연결 복구 무엇이든.

**규칙: 사용자가 만든 결과를 화면에만 두지 않는다.**

서버에 넣을 수 없는 사정이 있다면(배열 안 항목이라 읽고-고치고-쓰기가 필요하다
같은), 최소한 **다시 그리는 쪽이 그것을 알아야 한다.** 화면을 지우는 함수에
"지금 지우면 안 되는 상태" 를 묻는 자리를 만든다.

  shouldPreserveDailyLogMediaUi()
    - 분석이 도는 중인가
    - 업로드가 도는 중인가
    - 저장되지 않은 분석이 화면에 있는가

그리고 그 규칙이 새면 **콘솔이 말하게 한다.** 세 번 다 제보로만 알았다.

---

## 263. "이 기기에만 남긴다" 는 기기가 하나일 때의 셈이다 (2026-09-21)

제보: "폰에서 확인했던 배지 축하와 동의창이 새 컴퓨터로 로그인하니까 처음부터
다시 다 뜨네?"

축하를 확인한 사실은 localStorage 에만 있었다. 주석에 이유까지 적혀 있었다.

> 본 사실은 이 기기에만 남긴다. 서버에 쓰면 users 문서에 쓰기가 하나 더 늘고,
> 축하를 다른 기기에서 **한 번 더** 보는 것은 그만한 값이 아니다.

셈 자체는 틀리지 않았다. 틀린 것은 **"한 번 더"** 다. 기기를 바꾸면 한 번 더가
아니라 **전부 다시** 다. 배지는 몇 달에 걸쳐 쌓이므로, 새 기기의 첫 화면이 지난
축하로 뒤덮인다. 한 건의 비용으로 따져서 무시했는데 실제로는 N건이었다.

**규칙: 기기에 남길지 계정에 남길지는 "한 건의 값" 이 아니라 "기기를 바꾼 날
몇 건이 한꺼번에 오는가" 로 정한다.**

가르는 질문은 하나다. **이것은 이 브라우저의 형편인가, 그 사람의 사실인가.**

  브라우저의 형편 → localStorage
    마지막으로 보던 탭, 접어 둔 칸, 보내지 않은 초안, 사흘 미루기

  그 사람의 사실 → 계정
    확인한 축하, 받은 동의, 읽은 공지, 거절한 권유

옮길 때 두 가지를 같이 챙긴다.

- **모르는 상태와 없는 상태를 구분한다.** 빈 배열(본 적 없다)과 null(아직 안
  물어봤다)이 같으면, 계정 기록이 오기 전에 띄워 버려서 고치려던 그 장면이
  그대로 재현된다. 답을 듣기 전에는 쥐고 있는다 — 다만 **영영 기다리지는
  않는다.** 답이 없으면 기기 기록으로 간다. 축하를 삼키는 것보다 한 번 더 보는
  쪽이 낫다.
- **기기 기록을 지우지 않는다.** 연결이 끊긴 채 확인한 축하는 그쪽에만 있다.
  둘을 합쳐서 본다.

같은 제보의 동의창은 원인이 달랐다. 그쪽은 이미 계정 기준이다 — 기기 기록이
아니라 **쓰기가 닿지 않은 것**이 남아 있었다. 260·261 과 같은 줄기다.

---

## 264. 더하기만 하면 되는 배열을 읽어서 쓰지 않는다 (2026-09-21)

제보: "첫 미션, 나만의 미션 배지가 원래 본서버 내 계정에 있었는데 자물쇠가 되어
버렸네?"
...
이 일치가 범인을 가리킨다. 마감(`archiveWeeklyMission`)이 이렇게 썼다.

```js
const existingBadges = existingData.missionBadges || [];
const allBadges = [...new Set([...existingBadges, ...newBadges])];
updateData.missionBadges = allBadges;          // ← 배열째
```

읽고-합치고-**배열째** 쓰기는 **읽기가 답한 만큼만 남긴다.** 캐시가 답하든,
필드가 아직 없든, 짧게 답한 그 한 번에 답하지 않은 항목이 지워진다. 그리고
다시 계산되지 않는 배지는 영영 돌아오지 않는다. 620명 중 1명만 드러났는데,
99명이 배지가 하나뿐이라 잃을 것이 없었기 때문이다 — **드물게 보인 것이지
드물게 일어난 것이 아니다.**

**규칙: 더하기만 하는 배열은 `arrayUnion` 으로 쓴다. 읽지 않는다.**

읽지 않으면 지울 수가 없다. 덤으로 딸려 오는 것들이 많다.

- 경쟁 상태가 없어진다 — 두 기기가 같은 순간에 써도 서로를 지우지 않는다
- 읽기 실패를 다룰 갈래가 사라진다 (`saveWeeklyMissions` 의 8초 대기와
  "읽기 실패 시 배지 건너뛰기" 분기가 통째로 없어졌다)
- 느려서 생기는 버그가 없어진다

**읽기가 필요한 것과 아닌 것을 가른다.**

| 하려는 일 | 쓰는 법 |
|---|---|
| 항목 더하기 | `arrayUnion` — 읽지 않는다 |
| 항목 빼기 | `arrayRemove` — 읽지 않는다 |
| 특정 항목 고치기 | `runTransaction` (262 의 운동 분석) |
| 전체를 다시 쓰기 | 서버 읽기 + 트랜잭션. 정말 그래야 하는지 먼저 의심한다 |

**찾는 법:** `[...new Set([...existing`, `[...existing, '` 를 grep 한다.
읽은 배열에 더해서 통째로 쓰는 자리는 전부 같은 병을 앓는다.

**드러난 방식도 기록해 둔다.** 아무 로그도, 오류도 없었다. 회원이 자물쇠를 보고
말해 준 것이 유일한 신호였다. 260·261·262 와 같은 줄기다 —
**조용히 지워지는 것이 가장 늦게 발견된다.**

---

## 265. 캐시가 "모른다" 고 답한 것을 "아니다" 로 읽지 않는다 (2026-09-21)

제보: **"혹시 앱을 열자마자 약관 동의 상태를 못 받아와서 띄우는 건가?
스테이징에서 열자마자 또 약관 동의 떴는데 새로고침 하니까 안 떠."**

정확한 관찰이다. 같은 계정, 같은 기록인데 답하는 쪽이 다르다.

```
앱을 처음 열 때   연결이 아직 덜 섬  →  캐시가 답함  →  동의 창
새로고침          이미 선 연결       →  서버가 답함  →  안 뜸
```

**9/18 에 이미 절반은 막아 두었는데 나머지 절반으로 샜다.**

```js
if (firstTime && userDocFromCache) return;   // ← firstTime 일 때만
```

| 캐시의 답 | 뜻 | 9/18 판정 |
|---|---|---|
| 기록이 아예 없다 | 모른다 | 막힘 |
| 판본이 낡았다 | **모른다** | **통과** ← 여기로 샜다 |

둘 다 "모른다" 인데 하나만 막았다. 그때 시험에 이렇게 적어 두기까지 했다 —
*"개정 안내는 캐시로도 알 수 있으니 그 창까지 막으면 약관 개정을 알릴 길이
없어진다."* 걱정은 옳았고, 결론이 틀렸다.

**규칙: "서버가 답했는가" 는 한 갈래에만 묻는 조건이 아니다. 결정 전체의 전제다.**

가드를 `firstTime` 같은 **하위 조건에 붙이면 반드시 새는 갈래가 생긴다.**
결정 앞에 한 번 세운다.

```js
if (userDocFromCache) { scheduleConsentRecheck(user); return; }
```

**그리고 미루기가 봐주기가 되면 안 된다.** 이것이 9/18 의 진짜 실수였다.
막기만 하면 동의를 정말 받아야 하는 사람이 관문을 그냥 지나가고, 우리는 받지
않은 동의를 받은 것처럼 두게 된다. 막을 거면 **다시 물을 자리를 같이 만든다** —
연결을 세우고 서버에 한 번 더(`scheduleConsentRecheck`). 그것도 실패하면
다음 로그인에 다시 본다. 모르는 채로 묻지도, 모르는 채로 넘기지도 않는다.

**찾는 법:** `fromCache` 를 쓰는 자리를 전부 본다. 다른 조건과 `&&` 로 묶여
...
## 267. 탭으로 여는 오버레이는 그 탭에 닫힌다 (2026-09-22)

제보: "팝업에서 어떻게 하는지 보기 눌렀는데 반응 없고 배너에서 앱으로 열기
눌렀는데 반응이 없는데? **삼성인터넷에서 시험했어.**"

"반응 없음" 이라 코드가 안 돌았다고 생각하기 쉽다. 아니었다. **열렸다가 같은
손가락에 닫혔다.**

```
탭  →  touchstart → touchend → (버튼 click) → 안내가 화면에 붙음
                              → 그 탭의 click 이 이제 손가락 밑에 있는
                                 backdrop 으로 간다 → "바깥을 눌렀다" → 닫힘
```

브라우저에서 재 보니 탭 직후 그 자리의 요소가 정확히 `android-install-guide`
였고, 옛 방식은 그 click 한 번에 사라졌다.

**마우스로는 재현되지 않는다.** 그래서 데스크톱에서 열어 보고 "잘 되는데?" 하고
넘어가게 된다. 제보가 기기 이름을 알려 준 것이 단서였다.

**규칙: 바깥 클릭으로 닫는 오버레이를, 그 오버레이를 여는 이벤트 안에서 붙이지
않는다.**

두 겹으로 막는다. 하나만으로는 엔진마다 새는 자리가 다르다.

```js
const openedAt = Date.now();
setTimeout(() => element.addEventListener('click', onClick), 0);  // 이벤트가 끝난 뒤에 붙인다
if (Date.now() - openedAt < 400) return;                          // 그러고도 붙는 click 은 무시
```

**찾는 법:** `event.target === <자기 자신>` 으로 닫는 자리를 전부 grep 한다.
그중 **탭 핸들러 안에서 만들어지는 것**이 위험하다. 타이머로 뜨는 것(저장 뒤
공유 권유처럼)은 여는 이벤트가 없어서 안 걸린다 — 그래서 어떤 시트는 멀쩡하고
어떤 시트만 안 되는, 헷갈리는 모양으로 나타난다.

**증상을 말 그대로 믿지 않는다.** "반응 없음" 은 안 돌았다는 뜻이 아니라
"결과가 남지 않았다" 는 뜻이다. 둘은 아주 다른 곳을 보게 만든다.

---

## 268. 같은 문장이 네 군데 더 있었다 (2026-09-23)

복귀 메일의 발송 시점을 사흘째에서 이틀째로 옮기면서, 문구가 "최근 3일간" 이라고
박혀 있는 것을 고쳤다. **한 군데만.**

46~90일 캠페인을 보내기 직전에 회원이 물었다. "어떤 내용이 발송되게 되지?"
실제 메일을 뽑아 보니 67일 쉬신 분께 이렇게 나갈 참이었다.

> **7일 이상 기록이 없으셔서** 바쁘게 보내고 계신 것 같아요

템플릿이 두 말 × 두 단계 = **네 벌**이었고, 나는 한국어 3일 본문 하나만 고쳤다.
시험도 그 하나만 확인했다. 47명에게 나가기 직전이었다.
...
ko 7일 본문   놓침      ← "7일 이상"
ko 7일 요약   놓침
en 3일 본문   놓침      ← "last 3 days"
en 7일 본문   놓침      ← "7+ days"
en 7일 요약   놓침      ← "7 days or more"
```

**규칙: 같은 문장이 몇 벌인지 먼저 세고 고친다.** 다국어 × 단계 × 제목/본문/요약은
곱셈이다. 하나를 고쳤으면 나머지가 몇 개인지 grep 으로 센다 — 눈으로 훑지 않는다.

**시험도 같이 곱해야 한다.** 한 조합만 보는 시험은 "고쳤다" 를 증명하지 못하고
"한 군데는 고쳤다" 만 증명한다. 조합을 배열로 돌린다.
...
**그리고 보내기 전에 실물을 뽑아 본다.** 이번에 막은 것은 시험이 아니라 회원의
질문이었다. 나가는 것이 메일·알림·문자처럼 **되돌릴 수 없는 것**이면, 한 통을
렌더링해서 읽어 보는 일을 건너뛰지 않는다.

**덤으로 드러난 것:** 날짜가 맞았어도 "바쁘게 보내고 계신 것 같아요" 는 두 달 쉰
사람에게 할 말이 아니다. 사정을 지어내 붙이는 말이 된다. 짐작은 최근에 멀어진
분께만 어울린다(`describeWhy`, 14일 기준).

---

## 269. 한꺼번에 보내면 절반이 조용히 실패한다 (2026-09-23)

46~90일 복귀 캠페인을 눌렀다. 대상 47명, 실제로 나간 것 **22통.**

관제탑은 성공으로 보였다. 나간 통수를 세어 보고서야 알았다.

못 받은 25명이 누구인지가 원인을 알려 줬다.
...
**두 분포가 겹친다.** 조건으로 걸러진 것이면 한쪽으로 쏠렸을 텐데 그렇지 않다 —
무작위로 실패했다는 뜻이고, 그건 보내는 방식의 문제다.

```js
Promise.allSettled(targets.map(...))   // 47개 연결을 동시에 연다
```

Gmail 이 동시 연결을 끊는다. **자동 발송 경로는 처음부터 for 루프였고, 수동
경로만 병렬이었다.** 같은 일을 하는 두 경로가 다른 방식을 쓰고 있었다.

**규칙: 바깥 서비스로 나가는 N건은 한 줄로 세워 보낸다.** 메일·푸시·API 호출
전부. 병렬이 빠르지만, 빠른 대신 절반이 사라지면 그건 빠른 게 아니다.

**그리고 재시도가 가능해야 한다.** 22통이 나가고 25통이 실패했는데 다시 누르면
이미 받은 22명이 한 통 더 받는다. 그래서 **오늘 이미 보낸 사람은 건너뛴다.**
"예전에 받았으면 영영 제외" 가 아니라 **오늘만** 본다 — 캠페인은 자동 발송이
포기한 분들께 보내는 일이라, 과거 이력으로 막으면 캠페인 자체가 성립하지 않는다.

**드러난 방식도 적어 둔다.** 이번에도 화면은 아무 말이 없었다. 회원이
"잘 나갔는지 확인해 줘" 라고 해서 세어 봤고, 그래서 알았다. **보내는 일은
보낸 뒤에 센다.** 267·266 과 같은 줄기다 — 성공 화면과 실제 결과는 다른 것이다.

---

## 270. 시간이 지나면 틀려지는 값은 저장하지 않는다 (2026-09-23)

`users/{uid}.currentStreak` 은 **기록을 저장할 때만** 갱신됐다. 그만둔 사람에게는
갱신되는 순간이 오지 않으므로 값이 마지막 기록일에 멈춘 채 남았다.

```
currentStreak > 0 인 회원            121명
  어제·오늘 기록함 (연속이 살아 있음)  14명
  마지막 기록이 46일 넘게 지남         81명
  최악: 180일 전에 멈췄는데 값은 2일
```

**저장된 값의 뜻은 '오늘의 연속' 이 아니라 '`lastLogDate` 시점의 연속' 이었다.**
둘을 같은 것으로 읽는 자리가 여섯 곳 있었고, 그중 하나는 회원에게 그대로 나가는
처방 축하 카드였다 — 46일 쉰 분께 "30일 연속으로 기록하고 계십니다" 를 보낼 수
있었다.

### 리셋 자리를 찾다가 알게 된 것

처음에는 "어디서 0 으로 되돌려야 하나" 를 찾았다. 그런 자리가 없었다.

**값을 틀리게 만드는 것이 사건이 아니라 사건의 부재이기 때문이다.** 기록을 저장할
때 리셋하는 방법은 그만둔 사람에게 아무 일도 하지 않는다 — 그 사람은 저장을 안
한다. 매일 밤 전체를 쓸어도 다음 날이면 같은 문제로 돌아온다.

**규칙: 시간이 지나는 것만으로 틀려지는 값은, 저장된 사실이 아니라 읽을 때 구하는
파생값으로 다룬다.** 저장해야 한다면 **짝이 되는 시각을 반드시 함께 저장하고**,
읽을 때 둘을 같이 본다. 여기서는 `lastLogDate` 가 이미 있었다 —
`(currentStreak, lastLogDate)` 쌍은 스스로를 설명한다.

### 찾는 법

파생값을 담은 필드를 `grep` 한 다음, **쓰는 곳과 읽는 곳의 수를 센다.** 쓰는 곳이
하나뿐인데 읽는 곳이 여섯이면 거의 항상 이 모양이다. 이 필드는 쓰는 곳이
`refreshMilestones` 하나뿐이었다 — **앱이 불러 줘야 갱신되는 서버 파생값**이라는
뜻이었고, 앱을 안 여는 사람에게는 영영 갱신되지 않았다.

### 백필은 고치는 일이 아니다

낡은 107건을 0 으로 되돌리는 스크립트를 썼지만, **그건 치우는 일이지 고치는 일이
아니다.** 내일 또 누군가 쉬면 그 사람 값은 다시 낡는다. 고친 것은 읽는 쪽이다.
백필을 주기적으로 돌려서 문제를 덮지 않는다.
...
축하 카드에 "연속이 살아 있나" 확인을 덧대면서 처음엔 `awardedPoints` 합으로
판단했다. **주간 처방 대기열은 문서를 줄여 실어 보내느라 `awardedPoints` 를 아예
싣지 않는다.** 그대로 뒀으면 그 경로에서 축하가 **통째로 사라지고도 아무 신호가
없었을 것이다** — 테스트가 잡아 줬다.
...
## 272. 같은 창이 또 뜨면, 고치기 전에 서버에 그 기록이 있는지부터 본다 (2026-09-23)

"약관 동의 창이 또 떴다" 가 운영 앱에서 세 번 왔다 (9/18 v415, 9/20 v419,
9/23 v433). 그사이 두 번 고쳤다 — 9/18 저장 시한, 9/21 "캐시가 답하면 묻지 않는다".
9/21 수정의 근거가 된 제보("새로고침하면 안 떠")는 **스테이징**이었고, 운영
계정에는 그 전제가 맞지 않았다.

9/23 에 서버를 직접 읽었다. 이 계정의 동의 기록은 네 항목 모두
`firstAgreedAt = 2026-09-23T04:56:08Z` — 제보 1분 전이었다. 동의 창은 서버가
답했을 때만 열리고(`resolveLatestUserDocData` · `scheduleConsentRecheck`),
consents 를 지우는 경로는 앱·함수·관제탑·스크립트 어디에도 없다. 그러니
**그 전까지 운영 서버에는 기록이 없었다.** 창은 매번 맞게 떴고, 9/18·9/20 의
동의는 이 기기에서 서버에 닿지 않았다 (261 과 같은 끝나지 않는 쓰기).
운영 Firestore 는 메모리 캐시라 앱을 닫으면 대기 중인 쓰기는 사라진다.

같은 날 활동 회원 22명의 첫 동의일이 9/22~23 에 14명 몰려 있었다. 지워진 것이
아니다 — 9/21 수정이 서버에 한 번 더 묻게 되면서 **기록 없이 지나가던 회원을
그제야 붙잡은 것**이다. 지금은 최근 30일 기록 회원 37명 전원이 유효한 동의를 갖고 있다.

**규칙: 같은 증상이 다시 오면 코드보다 서버의 그 문서를 먼저 읽는다.**
화면이 틀렸다고 가정하고 고친 것이 두 번이었다. 데이터가 "화면이 맞았다" 고
말하고 있었다.

**다음에 또 뜬다면** 이번에는 기록이 서버에 있다(9/23 확인). 그때 창이 뜨면
그것은 처음으로 **지워진** 경우이므로 원인이 다르다 — 그 문서의 consents 부터 본다.

## 2026-09-24 — 해빛스쿨은 최석재가 만든 것 (채널이 만든 것이 아님)
- 실수: 소개 문구를 "유튜브 채널 「최석재의 골든타임」에서 만든 서비스"로 씀. 채널은 만든 주체가 아니다.
- 규칙: 만든 사람은 **응급의학과 전문의 최석재 본인**. 유튜브 채널 「최석재의 골든타임」(@Doctors0)은 그를 찾는 곳으로만 언급한다.
- 앱 문구·SNS 원고 모두 같은 원칙.

## 2026-09-24 — 브런치 글에는 초대 링크 위에 "초대 이미지"를 넣는다
- 실수: 브런치 2편을 원고(.md)만 보고 올려서, 1편에 있던 초대 이미지(「초대장 도착 — 당신의 건강을 위해 초대합니다」)를 빠뜨림. 사용자가 발견.
- 원인: 1편 게시 때 원고 밖에서 추가한 요소였는데 원고·todo 어디에도 적혀 있지 않았다.
- 규칙: **새 편을 올리기 전에 직전 편의 게시된 글 구조(이미지·링크 카드 위치)를 읽어 보고 같은 틀을 맞춘다.** 원고가 게시물의 전부가 아니다.
- 초대 이미지 원본: t1.kakaocdn.net/brunch/service/user/2wi/image/ByEVfn4-6d3ny9nearsRsbLJEj8.png — 브런치 편집기에 `<img src=...>` 를 paste 하면 브런치가 다시 올려 들어간다. 배치: 본문 → 초대 이미지 → 빈 줄 → "→ 초대 링크" (링크 카드는 브런치가 자동으로 붙임).

## 2026-09-24 — 배포는 커밋된 것에서 한다 (작업 폴더에서 하지 않는다)
- 실수: 스테이징 hosting 을 작업 폴더에서 배포해, 다른 세션이 커밋하지 않은 `marketing/` 의 홍보 초안(.txt·.jpg)이 스테이징 주소에 공개됐다. hosting 은 저장소 루트를 통째로 올리고, 제외 목록에 없는 것은 전부 나간다.
- 원인: 같은 저장소를 여러 세션이 동시에 쓰는데, `git status` 의 `??` 를 "내 것 아님"으로만 읽고 "배포에 실린다"로 읽지 않았다.
- 규칙: **배포 전 `git status` 에 커밋 안 된 파일이 있으면 HEAD 의 깨끗한 사본(`git worktree add --detach`)에서 배포한다.** 새 최상위 폴더가 생기면 `firebase.json` hosting 제외 목록에 들어가야 하는지 본다(`marketing/**` 는 `59ac05d` 에서 넣었다).

## 2026-09-26 — 동의 기록의 firstAgreedAt 은 "원래 없었다"의 증거가 아니다
- 실수: 정명희 님 제보(동의창이 불안정)를 보고 서버 동의 기록의 firstAgreedAt 이 제보 3분 전이라 "그전엔 기록이 없었다 → 창이 뜬 건 정상"으로 판단해 사람에게 넘겼다. 같은 날 사장님 계정에서도 같은 제보가 왔다.
- 원인: firstAgreedAt 은 **창을 연 순간 앱이 본 기록**을 기준으로 정해진다. 앱이 기록을 잘못 읽어 "없음"이라고 보면, 그날 시각이 새로 찍힌다. 판단이 틀렸을 때와 기록이 정말 없었을 때가 똑같이 보인다.
- 규칙: **동의 기록이 원래 있었는지는 읽기 시점 조회(readTime, 1시간 안)로만 확인한다.** 그 시간이 지났으면 "모름"으로 두고, 같은 증상이 반복되면 앱 쪽 판단을 의심한다. 9/24 에는 readTime 으로 서버 기록이 멀쩡했다는 것까지 확인했었다.
- 고친 것: 동의창은 서버 함수 `getMyConsents`(관리자 권한으로 직접 읽음)가 "없다"고 할 때만 띄운다. 기기 SDK 의 fromCache·getDocFromServer 로 두 번 막았는데도 새어 나왔다.
