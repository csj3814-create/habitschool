...
### Fixed
- Deployed the Firestore rules, which had been six days behind the code and were rejecting every consent write. `consents` was added to both user whitelists on 2026-08-09 when the consent system was built, but the live ruleset was still the one released on 2026-08-05 — rules are deliberately outside the hosting/functions deploy shortcut, so they had never gone out. Every write of a consent record failed with `permission-denied` for six weeks. Across 562 members, **zero** had a consent record, including the six who signed up after the feature shipped. The live ruleset now matches the file exactly.
- Stopped the member document write from swallowing its own failure. It carried the new signup's consent record and was wrapped in `.catch(() => {})`, so a rules rejection looked like success and nothing surfaced for six weeks. It now logs the code and message, and says explicitly when it was the consent that failed. Sign-in still proceeds — a logging failure should not lock everyone out.
